import { ListMultiServiceAvailabilityTimeSlotsRequest as ListMultiServiceAvailabilityTimeSlotsRequest$1, ListMultiServiceAvailabilityTimeSlotsResponse as ListMultiServiceAvailabilityTimeSlotsResponse$1, GetMultiServiceAvailabilityTimeSlotRequest as GetMultiServiceAvailabilityTimeSlotRequest$1, GetMultiServiceAvailabilityTimeSlotResponse as GetMultiServiceAvailabilityTimeSlotResponse$1 } from './index.typings.js';
...
/** @enumType */
type LocationTypeWithLiterals = LocationType | 'BUSINESS' | 'CUSTOM' | 'CUSTOMER';
...
interface AvailableResources {
    ...
    resources?: Resource[];
    /**
     * Number of resources needed to provide the service.
     * For example, a slot for the service named `double massage` requires 2 resources of type `Staff Member` and 1 resource of type `Room`.
     * @internal
     * @min 1
     */
...
interface ListEventTimeSlotsResponse {
    ...
    /**
     * Schedule-level information, one entry per schedule in `timeSlots`.
     *
     * Only courses are returned today. A course is a single schedule with many sessions, and a customer
     * enrolls in the whole course, so the answer is the same for every session of that course and is
     * returned here once instead of on each time slot. Returned only when the request's `eventFilter.type`
     * includes `COURSE`. Empty when no returned time slot belongs to a course.
     * @internal
     * @maxSize 1000
     */
...
/**
 * Enrollment status and date range of a schedule.
 *
 * Only courses have one today. A course is a single schedule with many sessions. A customer enrolls in
 * the whole course, not in one session, so this information belongs to the schedule and not to any
 * single time slot.
 */
interface ScheduleInfo {
    /**
     * ID of the service the schedule belongs to. Matches `timeSlot.serviceId`.
     * @format GUID
     */
    serviceId?: string | null;
    /**
     * ID of the schedule the information belongs to.
     * @format GUID
     */
    scheduleId?: string | null;
    /**
     * Whether the course still accepts enrollment, evaluated against the **first session of the course**.
     *
     * Unlike `timeSlot.bookingPolicyViolations`, this never takes into account whether a single session has
     * already started. A course whose booking policy allows booking after the start keeps `tooLateToBook` as
     * `false` here even once some of its sessions have passed.
     */
    enrollmentViolations?: BookingPolicyViolations;
    /** Start of the first session of the course. */
    firstSessionStart?: Date | null;
    /** End of the last session of the course. Not returned for a course whose sessions repeat with no end date. */
...
interface GetEventTimeSlotResponse {
    ...
    timeSlot?: TimeSlot;
    /**
     * Schedule-level information. Returned only when the requested time slot belongs to a course.
     * @internal
     */
...
interface ListAvailabilityTimeSlotsRequest {
     ...
     * @format GUID
     * @maxSize 135
     * @deprecated Resources to include in response.
     * If specified, returns up to 10 available resources out of the provided list.
     * Otherwise, returns time slots with empty `availableResources`.
     * Takes precedence over `resourceTypes`.
     * @replacedBy resource_types
    ...
    cursorPaging?: CursorPaging;
    /**
     * Whether to return all available resources in response.
     * @internal
     */
...
/** Gender narrowing for an "any staff member" selection. */
interface RequestedGenderValue {
    /** Gender the customer narrowed "any staff member" to. */
    gender?: RequestedGenderWithLiterals;
}
/**
 * Selectable genders. Staff members of any other gender are reachable
 * through plain "any staff member" or by name.
 */
declare enum RequestedGender {
    /**
     * Any male staff member.
...
interface GetAvailabilityTimeSlotRequest {
    ...
    location?: Location;
    /**
     * IDs of the [resources](https://dev.wix.com/docs/api-reference/business-solutions/bookings/resources/resources-v2/introduction) to check availability for.
     * If omitted, time slots are returned if there's at least 1 available resource.
     * Takes precedence over `resourceTypes`.
     * @internal
     * @format GUID
     * @maxSize 135
     * @deprecated IDs of the [resources](https://dev.wix.com/docs/api-reference/business-solutions/bookings/resources/resources-v2/introduction) to check availability for.
     * If omitted, time slots are returned if there's at least 1 available resource.
     * Takes precedence over `resourceTypes`.
     * @replacedBy resource_types
     * @targetRemovalDate 2025-09-01
     ...
     * @internal
     */
    customerChoices?: GetAvailabilityTimeSlotRequestCustomerChoices;
    /**
     * Resource types to filter time slots.
     * Only returns time slots that have these specific resource types available.
     * This filters the time slots themselves, unlike `includeResourceTypeIds` which only controls response details.
...
/** Gender narrowing for an "any staff member" selection. */
interface V2RequestedGenderValue {
    /** Gender the customer narrowed "any staff member" to. */
    gender?: RequestedGenderValueRequestedGenderWithLiterals;
}
/**
 * Selectable genders. Staff members of any other gender are reachable
 * through plain "any staff member" or by name.
 */
declare enum RequestedGenderValueRequestedGender {
    /**
     * Any male staff member.
     * @documentationMaturity preview
     */
...
/** @enumType */
type RequestedGenderValueRequestedGenderWithLiterals = RequestedGenderValueRequestedGender | 'ANY_MALE' | 'ANY_FEMALE';
interface GetAvailabilityTimeSlotRequestCustomerChoices {
     ...
     * Requires `genderPubliclyAccessible` to be enabled in the business's [staff member settings](https://dev.wix.com/docs/api-reference/business-solutions/bookings/staff-members/staff-member-settings/introduction). The call fails with `REQUESTED_GENDER_NOT_ALLOWED` when it's disabled.
     */
    requestedGender?: V2RequestedGenderValue;
}
...
declare enum ReasonCode {
    ...
    /**
     * The diagnosed range falls outside the dates the service itself is offered (its own working-hours date range).
     * @documentationMaturity preview
     * @internal
     */
    REQUESTED_DATE_OUTSIDE_SERVICE_AVAILABILITY_RANGE = "REQUESTED_DATE_OUTSIDE_SERVICE_AVAILABILITY_RANGE"
}
/** @enumType */
...
declare enum SuggestedAction {
    ...
    /** Review the date range the service is offered on (the service's own working hours). */
    CHECK_SERVICE_AVAILABILITY_RANGE = "CHECK_SERVICE_AVAILABILITY_RANGE"
}
/** @enumType */
type SuggestedActionWithLiterals = SuggestedAction | 'ASSIGN_STAFF_OR_RESOURCES' | 'CHECK_STAFF_WORKING_HOURS' | 'CHECK_WORK_LOCATIONS' | 'CHECK_SERVICE_LOCATIONS' | 'REDUCE_DURATION_OR_BUFFER' | 'CHECK_BLOCKED_TIME' | 'ASSIGN_RESOURCE_TO_SERVICE' | 'CHECK_SERVICE_AVAILABILITY_RANGE';
...
interface Service {
    ...
    serviceId?: string;
    /**
     * ID of the [resources](https://dev.wix.com/docs/rest/business-solutions/bookings/resources/resources-v2/introduction) to include in response.
     * Takes precedence over `resourceTypes`.
     * @internal
     * @format GUID
     * @maxSize 135
     * @deprecated ID of the [resources](https://dev.wix.com/docs/rest/business-solutions/bookings/resources/resources-v2/introduction) to include in response.
     * Takes precedence over `resourceTypes`.
     * @replacedBy resource_types
     * @targetRemovalDate 2025-09-01
     */
...
type ListMultiServiceAvailabilityTimeSlotsApplicationErrors = {
    ...
    code?: 'REQUESTED_GENDER_NOT_ALLOWED';
    description?: string;
    data?: Record<string, any>;
};
/** @docsIgnore */
...
declare function listMultiServiceAvailabilityTimeSlots(): __PublicMethodMetaInfo<'POST', {}, ListMultiServiceAvailabilityTimeSlotsRequest$1, ListMultiServiceAvailabilityTimeSlotsRequest, ListMultiServiceAvailabilityTimeSlotsResponse$1, ListMultiServiceAvailabilityTimeSlotsResponse>;
declare function getMultiServiceAvailabilityTimeSlot(): __PublicMethodMetaInfo<'POST', {}, GetMultiServiceAvailabilityTimeSlotRequest$1, GetMultiServiceAvailabilityTimeSlotRequest, GetMultiServiceAvailabilityTimeSlotResponse$1, GetMultiServiceAvailabilityTimeSlotResponse>;

export { type ApplicationError as ApplicationErrorOriginal, type AvailableResources as AvailableResourcesOriginal, type BookingPolicyViolations as BookingPolicyViolationsOriginal, type BulkActionMetadata as BulkActionMetadataOriginal, type BulkListAvailabilityTimeSlotsRequest as BulkListAvailabilityTimeSlotsRequestOriginal, type BulkListAvailabilityTimeSlotsResponse as BulkListAvailabilityTimeSlotsResponseOriginal, type BulkServiceTimeSlotsResult as BulkServiceTimeSlotsResultOriginal, type CursorPagingMetadata as CursorPagingMetadataOriginal, type CursorPaging as CursorPagingOriginal, type Cursors as CursorsOriginal, type CustomerChoices as CustomerChoicesOriginal, type DiagnoseAvailabilityRequest as DiagnoseAvailabilityRequestOriginal, type DiagnoseAvailabilityResponse as DiagnoseAvailabilityResponseOriginal, type DiagnosisReason as DiagnosisReasonOriginal, type EventInfo as EventInfoOriginal, type GetAvailabilityTimeSlotRequestCustomerChoices as GetAvailabilityTimeSlotRequestCustomerChoicesOriginal, type GetAvailabilityTimeSlotRequest as GetAvailabilityTimeSlotRequestOriginal, type GetAvailabilityTimeSlotResponse as GetAvailabilityTimeSlotResponseOriginal, type GetEventTimeSlotRequest as GetEventTimeSlotRequestOriginal, type GetEventTimeSlotResponse as GetEventTimeSlotResponseOriginal, type GetMultiServiceAvailabilityTimeSlotApplicationErrors as GetMultiServiceAvailabilityTimeSlotApplicationErrorsOriginal, type GetMultiServiceAvailabilityTimeSlotRequest as GetMultiServiceAvailabilityTimeSlotRequestOriginal, type GetMultiServiceAvailabilityTimeSlotResponse as GetMultiServiceAvailabilityTimeSlotResponseOriginal, type ItemMetadata as ItemMetadataOriginal, type ListAvailabilityTimeSlotEndOptionsRequest as ListAvailabilityTimeSlotEndOptionsRequestOriginal, type ListAvailabilityTimeSlotEndOptionsResponse as ListAvailabilityTimeSlotEndOptionsResponseOriginal, type ListAvailabilityTimeSlotsRequest as ListAvailabilityTimeSlotsRequestOriginal, type ListAvailabilityTimeSlotsResponse as ListAvailabilityTimeSlotsResponseOriginal, type ListEventTimeSlotsRequest as ListEventTimeSlotsRequestOriginal, type ListEventTimeSlotsResponse as ListEventTimeSlotsResponseOriginal, type ListMultiServiceAvailabilityTimeSlotsApplicationErrors as ListMultiServiceAvailabilityTimeSlotsApplicationErrorsOriginal, type ListMultiServiceAvailabilityTimeSlotsRequest as ListMultiServiceAvailabilityTimeSlotsRequestOriginal, type ListMultiServiceAvailabilityTimeSlotsResponse as ListMultiServiceAvailabilityTimeSlotsResponseOriginal, type Location as LocationOriginal, LocationType as LocationTypeOriginal, type LocationTypeWithLiterals as LocationTypeWithLiteralsOriginal, type NestedTimeSlot as NestedTimeSlotOriginal, type NonBookableReasons as NonBookableReasonsOriginal, ReasonCode as ReasonCodeOriginal, type ReasonCodeWithLiterals as ReasonCodeWithLiteralsOriginal, RequestedGender as RequestedGenderOriginal, type RequestedGenderValue as RequestedGenderValueOriginal, RequestedGenderValueRequestedGender as RequestedGenderValueRequestedGenderOriginal, type RequestedGenderValueRequestedGenderWithLiterals as RequestedGenderValueRequestedGenderWithLiteralsOriginal, type RequestedGenderWithLiterals as RequestedGenderWithLiteralsOriginal, type ResolvedContext as ResolvedContextOriginal, type Resource as ResourceOriginal, type ResourceType as ResourceTypeOriginal, type ScheduleInfo as ScheduleInfoOriginal, type Service as ServiceOriginal, type ServiceTimeSlots as ServiceTimeSlotsOriginal, SuggestedAction as SuggestedActionOriginal, type SuggestedActionWithLiterals as SuggestedActionWithLiteralsOriginal, type TimeSlot as TimeSlotOriginal, type V2CustomerChoices as V2CustomerChoicesOriginal, type V2RequestedGenderValue as V2RequestedGenderValueOriginal, type WaitingList as WaitingListOriginal, type __PublicMethodMetaInfo, getMultiServiceAvailabilityTimeSlot, listMultiServiceAvailabilityTimeSlots };
