...
interface SubmissionContactMapped {
    ...
    contactIdentityType?: ContactIdentityTypeWithLiterals;
}
...
/** @enumType */
type OptInLevelWithLiterals = OptInLevel | 'SINGLE_CONFIRMATION' | 'DOUBLE_CONFIRMATION';
declare enum ContactIdentityType {
    /** The returned contact ID belongs to a new or existing contact. */
    CONTACT = "CONTACT",
    /** The returned contact ID belongs to the currently logged-in site member. */
    MEMBER = "MEMBER",
    /** The returned contact ID belongs to a site member who is not currently logged in. */
    NOT_AUTHENTICATED_MEMBER = "NOT_AUTHENTICATED_MEMBER"
}
/** @enumType */
type ContactIdentityTypeWithLiterals = ContactIdentityType | 'CONTACT' | 'MEMBER' | 'NOT_AUTHENTICATED_MEMBER';
...
interface SearchSubmissionsForExtendedExportRequest {
    /**
     * Reconstructed into `CursorSearch` when `search` isn't set. Internal legacy path for export infra that requires `CursorQuery`.
     * @internal
     */
    query?: CursorQuery;
    /** Cursor search. Specify filters for both `namespace` and `formId`. */
    search?: CursorSearch;
}
...
interface SearchSubmissionsForExtendedExportResponse {
    /**
     * Submissions prepared for extended export; each item may include optional quiz-derived metadata.
     * @maxSize 100
     */
    exportRows?: SubmissionExtendedExportRow[];
    /** Paging metadata aligned with the delegated submission export search. */
    metadata?: CursorPagingMetadata;
}
/** Single submission row for extended export: core `submission` plus optional nested quiz result. */
interface SubmissionExtendedExportRow {
    /** Core submission entity including flattened field values for export. */
    submission?: FormSubmission;
    /**
     * Present when quiz grading is completed for this submission.
     * Omitted when the submission isn't graded as a quiz, no quiz result exists, or calculation isn't completed.
     */
    quizResult?: QuizExportResult;
}
/** Quiz grading summary for extended export. */
interface QuizExportResult {
    /** Total points earned for this submission. */
    score?: number | null;
    /** Maximum possible points for the graded form version. */
    ...
    /**
     * Quiz outcome label.
     * Returned values: `Passed` or `Failed`.
     * Omitted when the source outcome is unknown.
     * @maxLength 32
     */
    outcome?: string | null;
    /**
     * Per-field grading outcome keyed by form field target.
     * `true` means the answer was correct. `false` means any non-correct outcome.
     * @maxSize 200
     */
...
interface ValidationError {
    /**
     * Path to the value that failed validation, relative to the submission's values.
     *
     * For a top-level field this is the field's `target`, such as `email`. For a value nested inside a field, the path continues with `/`, such as `address/postal_code` for an address subfield, or `attachments/0/fileId` for an element of an array field.
     * @maxLength 1000
     */
    errorPath?: string;
    /**
     * Error type.
     *
     * Switch on this rather than parsing `errorMessage`, which carries the underlying validator's wording and isn't a stable contract.
     */
    errorType?: ErrorTypeWithLiterals;
    /**
     * Human readable description of the error, in English.
     *
     * This comes from the schema validator and is meant for debugging. It isn't localized and its wording can change, so don't parse it or show it to submitters. Use `errorType` to decide what to display.
     * @maxLength 1000
     */
    errorMessage?: string;
...
interface SearchSubmissionsByNamespaceForExportRequest {
    /** Legacy path: reconstructed into `CursorSearch` for `searchSubmissionsByNamespace` via `SubmissionMappers.searchForExportToSearchRequest`. */
    query?: CursorQuery;
    /** When set with a non-empty `CursorSearch` (filter / sort / free-text / paging), forwarded as `SearchSubmissionsByNamespaceRequest.search`. */
    search?: CursorSearch;
}
...
interface GetMediaUploadURLRequest {
    ...
    mimeType: string;
    /**
     * Human readable identifier used to reference a field to which file was uploaded. For example, `"address"`.
     * @internal
     * @maxLength 200
     */
...
interface DownloadSubmissionRequest {
    /**
...
interface ListFormattedSubmissionsRequest {
    /**
     * Namespace.
     * @minLength 10
     * @maxLength 50
...
interface ListFormattedSubmissionsResponse {
    /**
...
interface FormattedFormSubmission {
    /**
     * Submission ID.
     * @format GUID
     */
...
/** @docsIgnore */
type UpdateSubmissionValidationErrors = {
    ruleName?: 'SUBMISSION_VALIDATION';
};
 ...
 * @slug created
 */
declare function onSubmissionCreated(handler: (event: SubmissionCreatedEnvelope) => void | Promise<void>): void;
 ...
 * @slug deleted
 */
declare function onSubmissionDeleted(handler: (event: SubmissionDeletedEnvelope) => void | Promise<void>): void;
 ...
 */
declare function onSubmissionRemovedSubmissionFromTrash(handler: (event: SubmissionRemovedSubmissionFromTrashEnvelope) => void | Promise<void>): void;
 ...
 * @slug status_updated
 */
declare function onSubmissionStatusUpdated(handler: (event: SubmissionStatusUpdatedEnvelope) => void | Promise<void>): void;
 ...
 */
declare function onSubmissionContactMapped(handler: (event: SubmissionContactMappedEnvelope) => void | Promise<void>): void;
 ...
 */
declare function onSubmissionContactMappingSkipped(handler: (event: SubmissionContactMappingSkippedEnvelope) => void | Promise<void>): void;
 ...
 * @slug updated
 */
declare function onSubmissionUpdated(handler: (event: SubmissionUpdatedEnvelope) => void | Promise<void>): void;
/**
 ...
 */
declare function upsertContactFromSubmission(submissionId: string, options?: UpsertContactFromSubmissionOptions): Promise<NonNullablePaths<UpsertContactFromSubmissionResponse, `submitContactResponse.contactId` | `submitContactResponse.identityType` | `submitContactResponse.newContact`, 1>>;
 ...
 * @permissionId forms:v4:submission:create_submission
 * @applicableIdentity APP
 * @returns The created submission.
 * @fqn wix.forms.v4.FormSubmissionService.CreateSubmission
 */
declare function createSubmission(submission: NonNullablePaths<FormSubmission, `formId`, 0>, options?: CreateSubmissionOptions): Promise<NonNullablePaths<FormSubmission, `formId` | `namespace` | `status` | `seen` | `orderDetails.itemSubtotal` | `orderDetails.checkoutId` | `tags.tags.tagIds`, 2> & {
 ...
 * @public
 * @requiredField submissionId
 * @permissionId forms:v4:submission:get_submission
 * @applicableIdentity APP
 * @returns The retrieved submission.
 * @fqn wix.forms.v4.FormSubmissionService.GetSubmission
 */
declare function getSubmission(submissionId: string): Promise<NonNullablePaths<FormSubmission, `formId` | `namespace` | `status` | `seen` | `orderDetails.itemSubtotal` | `orderDetails.checkoutId` | `tags.tags.tagIds`, 2>>;
/**
 ...
 * @permissionId forms:v4:submission:update_submission
 * @applicableIdentity APP
 * @returns The updated submission.
 * @fqn wix.forms.v4.FormSubmissionService.UpdateSubmission
 */
declare function updateSubmission(_id: string, submission: NonNullablePaths<UpdateSubmission, `formId` | `revision`, 0>): Promise<NonNullablePaths<FormSubmission, `formId` | `namespace` | `status` | `seen` | `orderDetails.itemSubtotal` | `orderDetails.checkoutId` | `tags.tags.tagIds`, 2> & {
 ...
 * @public
 * @requiredField submissionId
 * @permissionId forms:v4:submission:confirm_submission
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.ConfirmSubmission
 */
declare function confirmSubmission(submissionId: string): Promise<NonNullablePaths<ConfirmSubmissionResponse, `submission.formId` | `submission.namespace` | `submission.status` | `submission.seen` | `submission.orderDetails.itemSubtotal` | `submission.orderDetails.checkoutId` | `submission.tags.tags.tagIds`, 3>>;
/**
 ...
 * @requiredField submissionId
 * @param options - Optional fields.
 * @permissionId forms:v4:submission:delete_submission
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.DeleteSubmission
 */
declare function deleteSubmission(submissionId: string, options?: DeleteSubmissionOptions): Promise<void>;
 ...
 * @permissionId forms:v4:submission:bulk_delete_submission
 * @applicableIdentity APP
 ...
 * @documentationMaturity preview
 * @requiredField submissionId
 * @permissionId forms:v4:submission:restore_submission_from_trash_bin
 * @applicableIdentity APP
 ...
 * @documentationMaturity preview
 * @requiredField submissionId
 * @permissionId forms:v4:submission:remove_submission_from_trash_bin
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.RemoveSubmissionFromTrashBin
 */
declare function removeSubmissionFromTrashBin(submissionId: string): Promise<void>;
/**
 ...
 * @documentationMaturity preview
 * @requiredField formId
 * @permissionId forms:v4:submission:bulk_remove_submission_from_trash_bin
 * @applicableIdentity APP
 ...
 * @documentationMaturity preview
 * @requiredField submissionId
 * @permissionId forms:v4:submission:get_deleted_submission
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.GetDeletedSubmission
 */
declare function getDeletedSubmission(submissionId: string): Promise<NonNullablePaths<GetDeletedSubmissionResponse, `submission.formId` | `submission.namespace` | `submission.status` | `submission.seen` | `submission.orderDetails.itemSubtotal` | `submission.orderDetails.checkoutId` | `submission.tags.tags.tagIds`, 3>>;
/**
 ...
 * @documentationMaturity preview
 * @requiredField query
 * @permissionId forms:v4:submission:query_submission
 * @applicableIdentity APP
...
interface FormSubmissionSearchSpec extends SearchSpec {
    paging: 'cursor';
    wql: [
        {
...
type CommonSearchWithEntityContext = Search<FormSubmission, FormSubmissionSearchSpec>;
type FormSubmissionSearch = {
    /**
    Cursor pointing to page of results.
    When requesting 'cursor_paging.cursor', no `filter`, `sort` or `search` can be provided.
    */
...
declare const utils: {
    search: _wix_sdk_types.SearchHelpers<FormSubmission, FormSubmissionSearchSpec, FormSubmissionSearch>;
    query: _wix_sdk_types.QueryHelpers<FormSubmission, FormSubmissionQuerySpec, FormSubmissionQuery>;
};
/**
 ...
 * @permissionId forms:v4:submission:query_submissions_by_namespace
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.QuerySubmissionsByNamespace
 */
declare function querySubmissionsByNamespace(options?: QuerySubmissionsByNamespaceOptions): SubmissionsQueryBuilder;
...
interface SubmissionsQueryBuilder {
     ...
     */
    eq: (propertyName: '_id' | 'formId' | 'namespace' | 'status' | '_createdDate' | '_updatedDate' | 'submitter.memberId' | 'submitter.visitorId' | 'submitter.applicationId' | 'submitter.userId' | 'seen' | 'orderDetails.checkoutId' | 'contactId' | 'childMetaSiteId', value: any) => SubmissionsQueryBuilder;
    /** @param propertyName - Property whose value is compared with `value`.
     * @param value - Value to compare against.
     */
    ne: (propertyName: '_id' | 'formId' | 'status' | '_createdDate' | '_updatedDate' | 'submitter.memberId' | 'submitter.visitorId' | 'submitter.applicationId' | 'submitter.userId' | 'seen' | 'orderDetails.checkoutId' | 'contactId' | 'childMetaSiteId', value: any) => SubmissionsQueryBuilder;
    /** @param propertyName - Property whose value is compared with `value`.
     * @param value - Value to compare against.
     */
    ...
    in: (propertyName: '_id' | 'formId' | 'status' | '_createdDate' | '_updatedDate' | 'submitter.memberId' | 'submitter.visitorId' | 'submitter.applicationId' | 'submitter.userId' | 'orderDetails.checkoutId' | 'contactId' | 'childMetaSiteId', value: any) => SubmissionsQueryBuilder;
    /** @param propertyNames - Properties used in the sort. To sort by multiple properties, pass properties as additional arguments. */
    ascending: (...propertyNames: Array<'_id' | 'formId' | 'status' | '_createdDate' | '_updatedDate' | 'submitter.memberId' | 'submitter.visitorId' | 'submitter.applicationId' | 'submitter.userId' | 'seen' | 'orderDetails.checkoutId' | 'contactId'>) => SubmissionsQueryBuilder;
    /** @param propertyNames - Properties used in the sort. To sort by multiple properties, pass properties as additional arguments. */
    descending: (...propertyNames: Array<'_id' | 'formId' | 'status' | '_createdDate' | '_updatedDate' | 'submitter.memberId' | 'submitter.visitorId' | 'submitter.applicationId' | 'submitter.userId' | 'seen' | 'orderDetails.checkoutId' | 'contactId'>) => SubmissionsQueryBuilder;
 ...
 * @permissionId forms:v4:submission:count_submissions_by_filter
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.CountSubmissionsByFilter
 */
declare function countSubmissionsByFilter(filter: Record<string, any>, options?: CountSubmissionsByFilterOptions): Promise<NonNullablePaths<CountSubmissionsByFilterResponse, `formsSubmissionsCount` | `formsSubmissionsCount.${number}.formId` | `formsSubmissionsCount.${number}.totalCount` | `formsSubmissionsCount.${number}.unseenCount`, 2>>;
 ...
 * @permissionId forms:v4:submission:count_submissions
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.CountSubmissions
 */
declare function countSubmissions(formIds: string[], namespace: string, options?: CountSubmissionsOptions): Promise<NonNullablePaths<CountSubmissionsResponse, `formsSubmissionsCount` | `formsSubmissionsCount.${number}.formId` | `formsSubmissionsCount.${number}.totalCount` | `formsSubmissionsCount.${number}.unseenCount`, 2>>;
 ...
 * @requiredField formIds
 * @requiredField namespace
 * @permissionId forms:v4:submission:count_deleted_submissions
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.CountDeletedSubmissions
 */
declare function countDeletedSubmissions(formIds: string[], namespace: string, options?: CountDeletedSubmissionsOptions): Promise<NonNullablePaths<CountDeletedSubmissionsResponse, `formsDeletedSubmissionsCount` | `formsDeletedSubmissionsCount.${number}.formId` | `formsDeletedSubmissionsCount.${number}.totalCount`, 2>>;
 ...
 * @permissionId forms:v4:submission:get_media_upload_url
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.GetMediaUploadURL
 */
declare function getMediaUploadUrl(formId: string, filename: string, mimeType: string): Promise<NonNullablePaths<GetMediaUploadURLResponse, `uploadUrl`, 0>>;
/**
 ...
 * @permissionId forms:v4:submission:bulk_mark_submissions_as_seen
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.BulkMarkSubmissionsAsSeen
 */
declare function bulkMarkSubmissionsAsSeen(ids: string[], formId: string): Promise<NonNullablePaths<BulkMarkSubmissionsAsSeenResponse, `jobId`, 0>>;
/**
 ...
 * @documentationMaturity preview
 * @requiredField submissionId
 * @permissionId forms:v4:submission:get_submission_download_url
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.GetSubmissionDownloadUrl
 */
declare function getSubmissionDownloadUrl(submissionId: string): Promise<NonNullablePaths<GetSubmissionDownloadUrlResponse, `document.readyDetails.downloadUrl` | `document.status`, 2>>;
/**
 ...
 */
declare function downloadSubmission(submissionId: string, accessToken: string): Promise<NonNullablePaths<RawHttpResponse, `body` | `headers` | `headers.${number}.key` | `headers.${number}.value`, 2>>;
/**
 ...
 * @documentationMaturity preview
 * @requiredField submissionId
 * @permissionId forms:v4:submission:get_formatted_submission
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.GetFormattedSubmission
 */
declare function getFormattedSubmission(submissionId: string): Promise<NonNullablePaths<GetFormattedSubmissionResponse, `submissionId` | `submissions` | `submissions.${number}.key` | `submissions.${number}.value` | `submissions.${number}.fieldId`, 2>>;
/**
 ...
 * @permissionId forms:v4:submission:update_extended_fields
 * @applicableIdentity APP
 ...
 * @permissionId forms:v4:submission:bulk_update_form_submission_tags
 * @applicableIdentity APP
 ...
 * @documentationMaturity preview
 * @requiredField filter
 * @permissionId forms:v4:submission:bulk_update_form_submission_tags_by_filter
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.BulkUpdateFormSubmissionTagsByFilter
 */
declare function bulkUpdateFormSubmissionTagsByFilter(filter: Record<string, any>, options?: BulkUpdateFormSubmissionTagsByFilterOptions): Promise<NonNullablePaths<BulkUpdateFormSubmissionTagsByFilterResponse, `jobId`, 0>>;
 ...
 * @documentationMaturity preview
 * @requiredField submission
 * @permissionId forms:v4:submission:validate_form_submission
 * @applicableIdentity APP
 * @fqn wix.forms.v4.FormSubmissionService.ValidateFormSubmission
 */
declare function validateFormSubmission(submission: FormSubmission, options?: ValidateFormSubmissionOptions): Promise<NonNullablePaths<ValidateFormSubmissionResponse, `violationErrors` | `violationErrors.${number}.name` | `violationErrors.${number}.description` | `violationErrors.${number}.ruleName`, 2>>;
...
export { SubmissionStatus as $, type BulkUpdateFormSubmissionTagsOptions as A, type BulkDeleteSubmissionOptions as B, type CreateSubmissionOptions as C, type DeleteSubmissionOptions as D, type BulkUpdateFormSubmissionTagsResponse as E, type FormSubmission as F, type GetDeletedSubmissionResponse as G, type BulkUpdateFormSubmissionTagsByFilterOptions as H, type BulkUpdateFormSubmissionTagsByFilterResponse as I, type ValidateFormSubmissionResponse as J, type SubmissionCreatedEnvelope as K, type ListDeletedSubmissionsOptions as L, type SubmissionDeletedEnvelope as M, type SubmissionRemovedSubmissionFromTrashEnvelope as N, type SubmissionStatusUpdatedEnvelope as O, type SubmissionContactMappedEnvelope as P, type QuerySubmissionOptions as Q, type RestoreSubmissionFromTrashBinResponse as R, type SearchSubmissionsByNamespaceResponse as S, type SubmissionContactMappingSkippedEnvelope as T, type UpsertContactFromSubmissionOptions as U, type ValidateFormSubmissionOptions as V, type SubmissionUpdatedEnvelope as W, type FormSubmissionQuery as X, type QuerySubmissionsByNamespaceOptions as Y, typedQuerySubmissionsByNamespace as Z, type SubmissionsQueryBuilder as _, type UpsertContactFromSubmissionResponse as a, LayerEffect as a$, OptInLevel as a0, ContactIdentityType as a1, SortOrder as a2, Mode as a3, WebhookIdentityType as a4, StringTypeFormatEnumFormat as a5, DayOfWeek as a6, ValidationFormat as a7, ProductType as a8, PriceType as a9, StylesPosition as aA, MapType as aB, ViewRole as aC, VoteRole as aD, PollLayoutType as aE, PollLayoutDirection as aF, PollDesignBackgroundType as aG, DecorationType as aH, FontType as aI, Variant as aJ, Position as aK, AspectRatio as aL, Resizing as aM, Placement as aN, CardStylesType as aO, CardStylesAlignment as aP, Layout as aQ, AppType as aR, InitialExpandedItems as aS, Direction as aT, VerticalAlignment as aU, NullValue as aV, ImageScalingScaling as aW, LayoutDataImagePosition as aX, LayoutDataBackgroundType as aY, Origin as aZ, BannerPosition as a_, FieldType as aa, FormatEnumFormat as ab, StringComponentType as ac, NodeType as ad, BackgroundType as ae, GradientType as af, WidthType as ag, PluginContainerDataAlignment as ah, ButtonDataType as ai, LinkTarget as aj, AnimationType as ak, EntranceEffectType as al, LoopEffectType as am, PointerEffectType as an, TextAlignment as ao, LineCap as ap, LineStyle as aq, Width as ar, DividerDataAlignment as as, ViewMode as at, LayoutType as au, Orientation as av, Crop as aw, ThumbnailsAlignment as ax, GIFType as ay, Source as az, type CreateSubmissionApplicationErrors as b, type Submitter as b$, Shape as b0, BackdropType as b1, VerticalAlignmentAlignment as b2, ResponsivenessBehaviour as b3, DesignTarget as b4, Scaling as b5, ImagePositionPosition as b6, CardDataBackgroundType as b7, ListStyle as b8, Indentation as b9, Tag as bA, ConfirmationLevel as bB, SubscriptionChannel as bC, ContactField as bD, DisplayFieldType as bE, OverrideEntityType as bF, Kind as bG, FormFieldContactInfoEmailInfoTag as bH, FormFieldContactInfoPhoneInfoTag as bI, AddressInfoTag as bJ, SubscriptionInfoOptInLevel as bK, FormFieldContactInfoContactField as bL, SpamFilterProtectionLevel as bM, RequiredIndicator as bN, RequiredIndicatorPlacement as bO, Target as bP, SubmitSuccessAction as bQ, ChangeableProperty as bR, OverrideEntityTypeEnumOverrideEntityType as bS, Operator as bT, ResultsDisplay as bU, ContactAutofill as bV, SubmissionAccess as bW, IdentityType as bX, ErrorType as bY, Status as bZ, SubmissionErrorType as b_, SmartBlockDataType as ba, ColumnSize as bb, ImagePosition as bc, Alignment as bd, ImageFit as be, NumberOfColumns as bf, FirstDayOfWeek as bg, NumberComponentType as bh, BooleanComponentType as bi, ItemType as bj, PropertiesTypeEnum as bk, ArrayComponentType as bl, WixFileComponentType as bm, UploadFileFormat as bn, PaymentComponentType as bo, ComponentType as bp, Type as bq, ObjectArrayComponentType as br, SchedulingComponentType as bs, WaitType as bt, Format as bu, MeetingType as bv, StaffStrategySelection as bw, InputType as bx, EmailInfoTag as by, PhoneInfoTag as bz, type CreateSubmissionValidationErrors as c, type QuantityLimit as c$, type SubmitterSubmitterOneOf as c0, type ExtendedFields as c1, type OrderDetails as c2, type PublicTags as c3, type TagList as c4, type AppointmentDetails as c5, type FormSubmissionStatusUpdatedEvent as c6, type RemovedSubmissionFromTrash as c7, type SubmissionContactMapped as c8, type MarketingSubscriptionDetails as c9, type Form as cA, type FormField as cB, type FormFieldStringType as cC, type FormFieldStringTypeFormatOptionsOneOf as cD, type StringErrorMessages as cE, type DateTimeAdvancedConstraints as cF, type Availability as cG, type DateRange as cH, type StringTypeDateTimeConstraints as cI, type StringTypePhoneConstraints as cJ, type StringTypeValidationMessages as cK, type FormFieldNumberType as cL, type NumberErrorMessages as cM, type IntegerType as cN, type FormFieldBooleanType as cO, type BooleanErrorMessages as cP, type FormFieldArrayType as cQ, type FormFieldObjectType as cR, type ObjectTypePropertiesType as cS, type ObjectTypePropertiesTypePropertiesTypeOneOf as cT, type ObjectErrorMessages as cU, type ArrayTypeArrayItems as cV, type ArrayTypeArrayItemsItemsOneOf as cW, type ArrayErrorMessages as cX, type PredefinedValidation as cY, type PredefinedValidationFormatOptionsOneOf as cZ, type PaymentType as c_, type SubmissionContactMappingSkipped as ca, type SearchSubmissionsForExtendedExportRequest as cb, type CursorQueryPagingMethodOneOf as cc, type Sorting as cd, type CursorPaging as ce, type CursorSearch as cf, type CursorSearchPagingMethodOneOf as cg, type SearchDetails as ch, type SearchSubmissionsForExtendedExportResponse as ci, type SubmissionExtendedExportRow as cj, type QuizExportResult as ck, type CursorPagingMetadata as cl, type Cursors as cm, type DomainEvent as cn, type DomainEventBodyOneOf as co, type EntityCreatedEvent as cp, type RestoreInfo as cq, type EntityUpdatedEvent as cr, type EntityDeletedEvent as cs, type ActionEvent as ct, type MessageEnvelope as cu, type IdentificationData as cv, type IdentificationDataIdOneOf as cw, type AccountInfo as cx, type CreateCheckoutFromSubmissionRequest as cy, type CreateCheckoutFromSubmissionRequestFormSchemaIdentifierOneOf as cz, type UpdateSubmission as d, type Video as d$, type FixedPriceOptions as d0, type DynamicPriceOptions as d1, type Product as d2, type ProductPriceOptionsOneOf as d3, type MultilineAddressValidation as d4, type FieldOverrides as d5, type FieldsOverrides as d6, type ObjectArrayType as d7, type NestedFormFieldOverrides as d8, type Validation as d9, type Background as dA, type PluginContainerData as dB, type PluginContainerDataWidth as dC, type PluginContainerDataWidthDataOneOf as dD, type Spoiler as dE, type Height as dF, type Styles as dG, type Link as dH, type LinkDataOneOf as dI, type Rel as dJ, type Animation as dK, type EntranceAnimation as dL, type EntranceEffect as dM, type LoopAnimation as dN, type LoopEffect as dO, type PointerEffect as dP, type CodeBlockData as dQ, type TextStyle as dR, type DividerData as dS, type DividerDataStyles as dT, type FileData as dU, type FileSource as dV, type FileSourceDataOneOf as dW, type PDFSettings as dX, type GalleryData as dY, type Media as dZ, type Image as d_, type ValidationValidationOneOf as da, type DataExtensionsDetails as db, type NestedFormOverrides as dc, type Field as dd, type FieldFieldTypeOptionsOneOf as de, type InputField as df, type InputFieldInputTypeOptionsOneOf as dg, type StringCorrectAnswersList as dh, type StringType as di, type StringTypeFormatOptionsOneOf as dj, type DateTimeConstraints as dk, type PhoneConstraints as dl, type ValidationMessages as dm, type StringQuizFieldSettings as dn, type TextInput as dp, type TextInputSemanticOptionsOneOf as dq, type RichContent as dr, type Node as ds, type NodeDataOneOf as dt, type NodeStyle as du, type ButtonData as dv, type Gradient as dw, type Stop as dx, type Border as dy, type Colors as dz, type UpdateSubmissionValidationErrors as e, type BorderColors as e$, type Item as e0, type ItemDataOneOf as e1, type GalleryOptions as e2, type GalleryOptionsLayout as e3, type ItemStyle as e4, type Thumbnails as e5, type GIFData as e6, type GIF as e7, type HeadingData as e8, type HTMLData as e9, type DecorationDataOneOf as eA, type AnchorData as eB, type ColorData as eC, type LinkData as eD, type MentionData as eE, type FontSizeData as eF, type SpoilerData as eG, type FontFamilyData as eH, type SketchData as eI, type AppEmbedData as eJ, type AppEmbedDataAppDataOneOf as eK, type BookingData as eL, type EventData as eM, type ButtonStyles as eN, type ImageStyles as eO, type RibbonStyles as eP, type CardStyles as eQ, type PricingData as eR, type VideoData as eS, type PlaybackOptions as eT, type EmbedData as eU, type Oembed as eV, type CollapsibleListData as eW, type TableData as eX, type Dimensions as eY, type TableCellData as eZ, type CellStyle as e_, type HTMLDataDataOneOf as ea, type ImageData as eb, type StylesBorder as ec, type ImageDataStyles as ed, type ImageDataCrop as ee, type LinkPreviewData as ef, type LinkPreviewDataStyles as eg, type MapData as eh, type MapSettings as ei, type ParagraphData as ej, type PollData as ek, type Permissions as el, type PollOption as em, type Settings as en, type PollLayout as eo, type OptionLayout as ep, type BackgroundGradient as eq, type PollDesignBackground as er, type PollDesignBackgroundBackgroundOneOf as es, type PollDesign as et, type OptionDesign as eu, type Poll as ev, type PollDataLayout as ew, type Design as ex, type TextData as ey, type Decoration as ez, type ConfirmSubmissionResponse as f, type ComponentsTags as f$, type BorderWidths as f0, type ListValue as f1, type AudioData as f2, type OrderedListData as f3, type BulletedListData as f4, type BlockquoteData as f5, type CaptionData as f6, type LayoutData as f7, type LayoutDataBackground as f8, type Layers as f9, type DropdownOption as fA, type DateTimeInput as fB, type PhoneInput as fC, type DateInput as fD, type TimeInput as fE, type DatePicker as fF, type ServicesDropdown as fG, type ServiceOption as fH, type Password as fI, type NumberCorrectAnswersList as fJ, type NumberType as fK, type NumberQuizFieldSettings as fL, type NumberInput as fM, type RatingInput as fN, type BooleanType as fO, type Checkbox as fP, type CorrectAnswersList as fQ, type ArrayType as fR, type ObjectType as fS, type PropertiesType as fT, type PropertiesTypePropertiesTypeOptionsOneOf as fU, type ArrayItems as fV, type ArrayItemsItemTypeOptionsOneOf as fW, type QuizFieldSettings as fX, type CheckboxGroup as fY, type Option as fZ, type OptionsMediaSettings as f_, type LayoutDataBackgroundImage as fa, type Banner as fb, type Backdrop as fc, type SectionDivider as fd, type LayoutCellData as fe, type ShapeData as ff, type ShapeDataStyles as fg, type CardData as fh, type CardDataBackground as fi, type BackgroundImage as fj, type TocData as fk, type SmartBlockData as fl, type SmartBlockCellData as fm, type CheckboxListData as fn, type ListItemNodeData as fo, type Metadata as fp, type DocumentStyle as fq, type TextNodeStyle as fr, type MediaItem as fs, type MediaItemMediaOneOf as ft, type MediaSettings as fu, type EmailOptions as fv, type RadioGroup as fw, type RadioGroupOption as fx, type CustomOption as fy, type Dropdown as fz, type BulkDeleteSubmissionResponse as g, type PageNavigationOptions as g$, type TagsOption as g0, type ServicesCheckboxGroup as g1, type FileType as g2, type FileUpload as g3, type Signature as g4, type ProductCheckboxGroup as g5, type ProductCheckboxGroupOption as g6, type DonationInput as g7, type DonationInputOption as g8, type PaymentInput as g9, type EmailInfo as gA, type PhoneInfo as gB, type AddressInfo as gC, type CustomFieldInfo as gD, type SubscriptionInfo as gE, type _String as gF, type _StringComponentTypeOptionsOneOf as gG, type _Number as gH, type _NumberComponentTypeOptionsOneOf as gI, type _Boolean as gJ, type _BooleanComponentTypeOptionsOneOf as gK, type _Array as gL, type _ArrayComponentTypeOptionsOneOf as gM, type _Object as gN, type WixFile as gO, type WixFileComponentTypeOptionsOneOf as gP, type Payment as gQ, type PaymentComponentTypeOptionsOneOf as gR, type Scheduling as gS, type SchedulingComponentTypeOptionsOneOf as gT, type Address as gU, type AddressComponentTypeOptionsOneOf as gV, type ObjectArray as gW, type ObjectArrayComponentTypeOptionsOneOf as gX, type DisplayField as gY, type DisplayFieldDisplayFieldTypeOptionsOneOf as gZ, type RichContentOptions as g_, type FixedPayment as ga, type MultilineAddress as gb, type AddressLine2 as gc, type DefaultCountryConfig as gd, type DefaultCountryConfigOptionsOneOf as ge, type FieldsSettings as gf, type Repeater as gg, type FormLayout as gh, type BreakPoint as gi, type ItemLayout as gj, type ItemLayoutItemOneOf as gk, type Group as gl, type Margin as gm, type Section as gn, type Appointment as go, type AppointmentFormatInfoOneOf as gp, type WaitOptions as gq, type Location as gr, type LocationLocationInfoOneOf as gs, type RebookingPolicy as gt, type RebookingPolicyOptionsOneOf as gu, type InPersonOptions as gv, type VideoConferenceOptions as gw, type PhoneOptions as gx, type FormFieldContactInfo as gy, type FormFieldContactInfoAdditionalInfoOneOf as gz, type BulkRemoveSubmissionFromTrashBinOptions as h, type GetSubmissionByCheckoutIdRequest as h$, type Step as h0, type FormRule as h1, type FormOverride as h2, type FormProperties as h3, type PostSubmissionTriggers as h4, type UpsertContact as h5, type V4FormFieldContactInfo as h6, type V4FormFieldContactInfoAdditionalInfoOneOf as h7, type FormFieldContactInfoEmailInfo as h8, type FormFieldContactInfoPhoneInfo as h9, type TagsTagList as hA, type QuizSettings as hB, type QuizSettingsPassingCriteriaOneOf as hC, type QuizSettingsResultsDisplayOptionsOneOf as hD, type PassFailMessages as hE, type CreateCheckoutFromSubmissionResponse as hF, type Checkout as hG, type IsFormSubmittableRequest as hH, type IsFormSubmittableResponse as hI, type Empty as hJ, type UpsertContactFromSubmissionRequest as hK, type SubmitContactResponse as hL, type CreateSubmissionRequest as hM, type CreateSubmissionResponse as hN, type SubmissionValidationErrorsDetails as hO, type ValidationError as hP, type CreateSubmissionBySubmitterRequest as hQ, type CreateSubmissionBySubmitterResponse as hR, type BulkCreateSubmissionBySubmitterRequest as hS, type BulkCreateSubmissionBySubmitterData as hT, type BulkCreateSubmissionBySubmitterResponse as hU, type BulkSubmissionResult as hV, type ItemMetadata as hW, type ApplicationError as hX, type BulkActionMetadata as hY, type GetSubmissionRequest as hZ, type GetSubmissionResponse as h_, type FormFieldContactInfoAddressInfo as ha, type FormFieldContactInfoCustomFieldInfo as hb, type FormFieldContactInfoSubscriptionInfo as hc, type NestedForm as hd, type LimitationRule as he, type RequiredIndicatorProperties as hf, type SubmitSettings as hg, type SubmitSettingsSubmitSuccessActionOptionsOneOf as hh, type ThankYouMessageOptions as hi, type RedirectOptions as hj, type PopupOptions as hk, type FieldGroup as hl, type Rule as hm, type RequiredOptions as hn, type HiddenOptions as ho, type AllowedValuesOptions as hp, type FieldOverride as hq, type FieldOverridePropertyTypeOptionsOneOf as hr, type ConditionNode as hs, type ConditionNodeNodeOneOf as ht, type AndCondition as hu, type OrCondition as hv, type Condition as hw, type RuleFormOverride as hx, type RuleFormOverrideEntityTypeOptionsOneOf as hy, type Tags as hz, type BulkRemoveSubmissionFromTrashBinResponse as i, type SortOrderWithLiterals as i$, type GetSubmissionByCheckoutIdResponse as i0, type UpdateSubmissionRequest as i1, type UpdateSubmissionResponse as i2, type ConfirmSubmissionRequest as i3, type DeleteSubmissionRequest as i4, type DeleteSubmissionResponse as i5, type BulkDeleteSubmissionRequest as i6, type BulkDeleteSubmissionResult as i7, type RestoreSubmissionFromTrashBinRequest as i8, type RemoveSubmissionFromTrashBinRequest as i9, type DownloadSubmissionRequest as iA, type HeadersEntry as iB, type GetFormattedSubmissionRequest as iC, type FormattedSubmission as iD, type ListFormattedSubmissionsRequest as iE, type ListFormattedSubmissionsResponse as iF, type FormattedFormSubmission as iG, type UpdateExtendedFieldsRequest as iH, type BulkUpdateFormSubmissionTagsRequest as iI, type BulkUpdateFormSubmissionTagsResult as iJ, type BulkUpdateFormSubmissionTagsByFilterRequest as iK, type ValidateFormSubmissionRequest as iL, type FieldViolation as iM, type FieldViolationErrorDataOneOf as iN, type SubmissionValidationErrors as iO, type SubmissionValidationError as iP, type SubmissionValidationErrorErrorMessageOneOf as iQ, type BaseEventMetadata as iR, type EventMetadata as iS, type AccountInfoMetadata as iT, type FormSubmissionSearchSpec as iU, type SubmissionsQueryResult as iV, type FormSubmissionQuerySpec as iW, utils as iX, type SubmissionStatusWithLiterals as iY, type OptInLevelWithLiterals as iZ, type ContactIdentityTypeWithLiterals as i_, type RemoveSubmissionFromTrashBinResponse as ia, type BulkRemoveSubmissionFromTrashBinRequest as ib, type BulkRemoveSubmissionFromTrashBinResult as ic, type ListDeletedSubmissionsRequest as id, type GetDeletedSubmissionRequest as ie, type QuerySubmissionRequest as ig, type SearchSubmissionsByNamespaceRequest as ih, type SearchSubmissionsByNamespaceForExportRequest as ii, type SearchSubmissionsByNamespaceForExportResponse as ij, type QuerySubmissionsByNamespaceRequest as ik, type QuerySubmissionsByNamespaceResponse as il, type QuerySubmissionsByNamespaceForExportRequest as im, type QuerySubmissionsByNamespaceForExportResponse as io, type CountSubmissionsByFilterRequest as ip, type FormSubmissionsCount as iq, type CountSubmissionsRequest as ir, type CountDeletedSubmissionsRequest as is, type FormDeletedSubmissionsCount as it, type GetMediaUploadURLRequest as iu, type BulkMarkSubmissionsAsSeenRequest as iv, type GetSubmissionDownloadUrlRequest as iw, type SubmissionDocument as ix, type SubmissionDocumentDocumentOneOf as iy, type DocumentReady as iz, type ListDeletedSubmissionsResponse as j, type VerticalAlignmentAlignmentWithLiterals as j$, type ModeWithLiterals as j0, type WebhookIdentityTypeWithLiterals as j1, type StringTypeFormatEnumFormatWithLiterals as j2, type DayOfWeekWithLiterals as j3, type ValidationFormatWithLiterals as j4, type ProductTypeWithLiterals as j5, type PriceTypeWithLiterals as j6, type FieldTypeWithLiterals as j7, type FormatEnumFormatWithLiterals as j8, type StringComponentTypeWithLiterals as j9, type VoteRoleWithLiterals as jA, type PollLayoutTypeWithLiterals as jB, type PollLayoutDirectionWithLiterals as jC, type PollDesignBackgroundTypeWithLiterals as jD, type DecorationTypeWithLiterals as jE, type FontTypeWithLiterals as jF, type VariantWithLiterals as jG, type PositionWithLiterals as jH, type AspectRatioWithLiterals as jI, type ResizingWithLiterals as jJ, type PlacementWithLiterals as jK, type CardStylesTypeWithLiterals as jL, type CardStylesAlignmentWithLiterals as jM, type LayoutWithLiterals as jN, type AppTypeWithLiterals as jO, type InitialExpandedItemsWithLiterals as jP, type DirectionWithLiterals as jQ, type VerticalAlignmentWithLiterals as jR, type NullValueWithLiterals as jS, type ImageScalingScalingWithLiterals as jT, type LayoutDataImagePositionWithLiterals as jU, type LayoutDataBackgroundTypeWithLiterals as jV, type OriginWithLiterals as jW, type BannerPositionWithLiterals as jX, type LayerEffectWithLiterals as jY, type ShapeWithLiterals as jZ, type BackdropTypeWithLiterals as j_, type NodeTypeWithLiterals as ja, type BackgroundTypeWithLiterals as jb, type GradientTypeWithLiterals as jc, type WidthTypeWithLiterals as jd, type PluginContainerDataAlignmentWithLiterals as je, type ButtonDataTypeWithLiterals as jf, type LinkTargetWithLiterals as jg, type AnimationTypeWithLiterals as jh, type EntranceEffectTypeWithLiterals as ji, type LoopEffectTypeWithLiterals as jj, type PointerEffectTypeWithLiterals as jk, type TextAlignmentWithLiterals as jl, type LineCapWithLiterals as jm, type LineStyleWithLiterals as jn, type WidthWithLiterals as jo, type DividerDataAlignmentWithLiterals as jp, type ViewModeWithLiterals as jq, type LayoutTypeWithLiterals as jr, type OrientationWithLiterals as js, type CropWithLiterals as jt, type ThumbnailsAlignmentWithLiterals as ju, type GIFTypeWithLiterals as jv, type SourceWithLiterals as jw, type StylesPositionWithLiterals as jx, type MapTypeWithLiterals as jy, type ViewRoleWithLiterals as jz, type CursorQuery as k, onSubmissionDeleted as k$, type ResponsivenessBehaviourWithLiterals as k0, type DesignTargetWithLiterals as k1, type ScalingWithLiterals as k2, type ImagePositionPositionWithLiterals as k3, type CardDataBackgroundTypeWithLiterals as k4, type ListStyleWithLiterals as k5, type IndentationWithLiterals as k6, type SmartBlockDataTypeWithLiterals as k7, type ColumnSizeWithLiterals as k8, type ImagePositionWithLiterals as k9, type ContactFieldWithLiterals as kA, type DisplayFieldTypeWithLiterals as kB, type OverrideEntityTypeWithLiterals as kC, type KindWithLiterals as kD, type FormFieldContactInfoEmailInfoTagWithLiterals as kE, type FormFieldContactInfoPhoneInfoTagWithLiterals as kF, type AddressInfoTagWithLiterals as kG, type SubscriptionInfoOptInLevelWithLiterals as kH, type FormFieldContactInfoContactFieldWithLiterals as kI, type SpamFilterProtectionLevelWithLiterals as kJ, type RequiredIndicatorWithLiterals as kK, type RequiredIndicatorPlacementWithLiterals as kL, type TargetWithLiterals as kM, type SubmitSuccessActionWithLiterals as kN, type ChangeablePropertyWithLiterals as kO, type OverrideEntityTypeEnumOverrideEntityTypeWithLiterals as kP, type OperatorWithLiterals as kQ, type ResultsDisplayWithLiterals as kR, type ContactAutofillWithLiterals as kS, type SubmissionAccessWithLiterals as kT, type IdentityTypeWithLiterals as kU, type ErrorTypeWithLiterals as kV, type StatusWithLiterals as kW, type SubmissionErrorTypeWithLiterals as kX, type CommonSearchWithEntityContext as kY, type CommonQueryWithEntityContext as kZ, onSubmissionCreated as k_, type AlignmentWithLiterals as ka, type ImageFitWithLiterals as kb, type NumberOfColumnsWithLiterals as kc, type FirstDayOfWeekWithLiterals as kd, type NumberComponentTypeWithLiterals as ke, type BooleanComponentTypeWithLiterals as kf, type ItemTypeWithLiterals as kg, type PropertiesTypeEnumWithLiterals as kh, type ArrayComponentTypeWithLiterals as ki, type WixFileComponentTypeWithLiterals as kj, type UploadFileFormatWithLiterals as kk, type PaymentComponentTypeWithLiterals as kl, type ComponentTypeWithLiterals as km, type TypeWithLiterals as kn, type ObjectArrayComponentTypeWithLiterals as ko, type SchedulingComponentTypeWithLiterals as kp, type WaitTypeWithLiterals as kq, type FormatWithLiterals as kr, type MeetingTypeWithLiterals as ks, type StaffStrategySelectionWithLiterals as kt, type InputTypeWithLiterals as ku, type EmailInfoTagWithLiterals as kv, type PhoneInfoTagWithLiterals as kw, type TagWithLiterals as kx, type ConfirmationLevelWithLiterals as ky, type SubscriptionChannelWithLiterals as kz, type QuerySubmissionResponse as l, onSubmissionRemovedSubmissionFromTrash as l0, onSubmissionStatusUpdated as l1, onSubmissionContactMapped as l2, onSubmissionContactMappingSkipped as l3, onSubmissionUpdated as l4, upsertContactFromSubmission as l5, createSubmission as l6, getSubmission as l7, updateSubmission as l8, confirmSubmission as l9, deleteSubmission as la, bulkDeleteSubmission as lb, restoreSubmissionFromTrashBin as lc, removeSubmissionFromTrashBin as ld, bulkRemoveSubmissionFromTrashBin as le, listDeletedSubmissions as lf, getDeletedSubmission as lg, querySubmission as lh, querySubmissionsByNamespace as li, countSubmissionsByFilter as lj, countSubmissions as lk, countDeletedSubmissions as ll, getMediaUploadUrl as lm, bulkMarkSubmissionsAsSeen as ln, getSubmissionDownloadUrl as lo, downloadSubmission as lp, getFormattedSubmission as lq, updateExtendedFields as lr, bulkUpdateFormSubmissionTags as ls, bulkUpdateFormSubmissionTagsByFilter as lt, validateFormSubmission as lu, type FormSubmissionSearch as m, type CountSubmissionsByFilterOptions as n, type CountSubmissionsByFilterResponse as o, type CountSubmissionsOptions as p, type CountSubmissionsResponse as q, type CountDeletedSubmissionsOptions as r, type CountDeletedSubmissionsResponse as s, type GetMediaUploadURLResponse as t, type BulkMarkSubmissionsAsSeenResponse as u, type GetSubmissionDownloadUrlResponse as v, type RawHttpResponse as w, type GetFormattedSubmissionResponse as x, type UpdateExtendedFieldsOptions as y, type UpdateExtendedFieldsResponse as z };
