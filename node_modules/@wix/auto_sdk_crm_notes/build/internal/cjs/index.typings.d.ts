 ...
 */
declare function createNote(note: NonNullablePaths<Note, `contactId`, 0>): Promise<NonNullablePaths<Note, `type` | `source.sourceType`, 1>>;
/**
 ...
 */
declare function getNote(noteId: string): Promise<NonNullablePaths<Note, `type` | `source.sourceType`, 1>>;
/**
 * Updates a note.
 *
 ...
 */
declare function updateNote(_id: string, note: NonNullablePaths<UpdateNote, `revision`, 0>): Promise<NonNullablePaths<Note, `type` | `source.sourceType`, 1>>;
 ...
 * @requiredField query
 */
declare function typedQueryNotes(query: NoteQuery, options?: QueryNotesOptions): Promise<NonNullablePaths<QueryNotesResponse, `notes` | `notes.${number}.type` | `notes.${number}.source.sourceType`, 3> & {
    __applicationErrorsType?: QueryNotesApplicationErrors;
}>;
interface NoteQuerySpec extends QuerySpec {
    paging: 'cursor';
    wql: [
        {
            fields: ['_id'];
...
type CommonQueryWithEntityContext = Query<Note, NoteQuerySpec>;
type NoteQuery = {
    /**
    Cursor token pointing to a page of results. Not used in the first request. Following requests use the cursor token and not `filter` or `sort`.
    */
...
declare const utils: {
    query: _wix_sdk_types.QueryHelpers<Note, NoteQuerySpec, NoteQuery>;
};
