// src/contacts-v4-label-labels.public.ts
import { renameKeysFromRESTResponseToSDKResponse as renameKeysFromRESTResponseToSDKResponse2 } from "@wix/sdk-runtime/rename-all-nested-keys";
import { transformRESTTimestampToSDKTimestamp as transformRESTTimestampToSDKTimestamp2 } from "@wix/sdk-runtime/transformations/timestamp";
import { transformPaths as transformPaths3 } from "@wix/sdk-runtime/transformations/transform-paths";
import { EventDefinition } from "@wix/sdk-types";

// src/contacts-v4-label-labels.universal.ts
import { transformError as sdkTransformError } from "@wix/sdk-runtime/transform-error";
import { queryBuilder } from "@wix/sdk-runtime/query-builder";
import {
  renameKeysFromSDKRequestToRESTRequest,
  renameKeysFromRESTResponseToSDKResponse
} from "@wix/sdk-runtime/rename-all-nested-keys";

// src/contacts-v4-label-labels.http.ts
import { toURLSearchParams } from "@wix/sdk-runtime/rest-modules";
import { transformSDKTimestampToRESTTimestamp } from "@wix/sdk-runtime/transformations/timestamp";
import { transformRESTTimestampToSDKTimestamp } from "@wix/sdk-runtime/transformations/timestamp";
import { transformPaths } from "@wix/sdk-runtime/transformations/transform-paths";
import { resolveUrl } from "@wix/sdk-runtime/rest-modules";
function resolveComWixpressContactsLabelsV4ContactLabelsServiceV4Url(opts) {
  const domainToMappings = {
    "www.wixapis.com": [
      {
        srcPath: "/contacts/v4/labels",
        destPath: "/v4/labels"
      }
    ],
    "www._base_domain_": [
      {
        srcPath: "/_api/contacts/v4/labels",
        destPath: "/v4/labels"
      }
    ],
    "manage._base_domain_": [
      {
        srcPath: "/_api/contacts/v4/labels",
        destPath: "/v4/labels"
      }
    ],
    "editor._base_domain_": [
      {
        srcPath: "/_api/contacts/v4/labels",
        destPath: "/v4/labels"
      }
    ],
    "blocks._base_domain_": [
      {
        srcPath: "/_api/contacts/v4/labels",
        destPath: "/v4/labels"
      }
    ],
    "create.editorx": [
      {
        srcPath: "/_api/contacts/v4/labels",
        destPath: "/v4/labels"
      }
    ],
    _: [
      {
        srcPath: "/_api/contacts/v4/labels",
        destPath: "/v4/labels"
      }
    ],
    "*.dev.wix-code.com": [
      {
        srcPath: "/_api/contacts/v4/labels",
        destPath: "/v4/labels"
      }
    ]
  };
  return resolveUrl(Object.assign(opts, { domainToMappings }));
}
var PACKAGE_NAME = "@wix/auto_sdk_crm_labels";
function listLabels(payload) {
  function __listLabels({ host }) {
    const metadata = {
      entityFqdn: "wix.contacts.v4.label",
      method: "GET",
      methodFqn: "com.wixpress.contacts.labels.v4.ContactLabelsServiceV4.ListLabels",
      packageName: PACKAGE_NAME,
      migrationOptions: {
        optInTransformResponse: true
      },
      url: resolveComWixpressContactsLabelsV4ContactLabelsServiceV4Url({
        protoPath: "/v4/labels",
        data: payload,
        host
      }),
      params: toURLSearchParams(payload),
      transformResponse: (payload2) => transformPaths(payload2, [
        {
          transformFn: transformRESTTimestampToSDKTimestamp,
          paths: [
            { path: "labels.createdDate" },
            { path: "labels.updatedDate" }
          ]
        }
      ])
    };
    return metadata;
  }
  return __listLabels;
}
function findOrCreateLabel(payload) {
  function __findOrCreateLabel({ host }) {
    const metadata = {
      entityFqdn: "wix.contacts.v4.label",
      method: "POST",
      methodFqn: "com.wixpress.contacts.labels.v4.ContactLabelsServiceV4.FindOrCreateLabel",
      packageName: PACKAGE_NAME,
      migrationOptions: {
        optInTransformResponse: true
      },
      url: resolveComWixpressContactsLabelsV4ContactLabelsServiceV4Url({
        protoPath: "/v4/labels",
        data: payload,
        host
      }),
      data: payload,
      transformResponse: (payload2) => transformPaths(payload2, [
        {
          transformFn: transformRESTTimestampToSDKTimestamp,
          paths: [
            { path: "label.createdDate" },
            { path: "label.updatedDate" }
          ]
        }
      ])
    };
    return metadata;
  }
  return __findOrCreateLabel;
}
function getLabel(payload) {
  function __getLabel({ host }) {
    const metadata = {
      entityFqdn: "wix.contacts.v4.label",
      method: "GET",
      methodFqn: "com.wixpress.contacts.labels.v4.ContactLabelsServiceV4.GetLabel",
      packageName: PACKAGE_NAME,
      migrationOptions: {
        optInTransformResponse: true
      },
      url: resolveComWixpressContactsLabelsV4ContactLabelsServiceV4Url({
        protoPath: "/v4/labels/{key}",
        data: payload,
        host
      }),
      params: toURLSearchParams(payload),
      transformResponse: (payload2) => transformPaths(payload2, [
        {
          transformFn: transformRESTTimestampToSDKTimestamp,
          paths: [
            { path: "label.createdDate" },
            { path: "label.updatedDate" }
          ]
        }
      ])
    };
    return metadata;
  }
  return __getLabel;
}
function updateLabel(payload) {
  function __updateLabel({ host }) {
    const serializedData = transformPaths(payload, [
      {
        transformFn: transformSDKTimestampToRESTTimestamp,
        paths: [{ path: "label.createdDate" }, { path: "label.updatedDate" }]
      }
    ]);
    const metadata = {
      entityFqdn: "wix.contacts.v4.label",
      method: "PATCH",
      methodFqn: "com.wixpress.contacts.labels.v4.ContactLabelsServiceV4.UpdateLabel",
      packageName: PACKAGE_NAME,
      migrationOptions: {
        optInTransformResponse: true
      },
      url: resolveComWixpressContactsLabelsV4ContactLabelsServiceV4Url({
        protoPath: "/v4/labels/{label.key}",
        data: serializedData,
        host
      }),
      data: serializedData,
      transformResponse: (payload2) => transformPaths(payload2, [
        {
          transformFn: transformRESTTimestampToSDKTimestamp,
          paths: [
            { path: "label.createdDate" },
            { path: "label.updatedDate" }
          ]
        }
      ])
    };
    return metadata;
  }
  return __updateLabel;
}
function deleteLabel(payload) {
  function __deleteLabel({ host }) {
    const metadata = {
      entityFqdn: "wix.contacts.v4.label",
      method: "DELETE",
      methodFqn: "com.wixpress.contacts.labels.v4.ContactLabelsServiceV4.DeleteLabel",
      packageName: PACKAGE_NAME,
      migrationOptions: {
        optInTransformResponse: true
      },
      url: resolveComWixpressContactsLabelsV4ContactLabelsServiceV4Url({
        protoPath: "/v4/labels/{key}",
        data: payload,
        host
      }),
      params: toURLSearchParams(payload)
    };
    return metadata;
  }
  return __deleteLabel;
}
function queryLabels(payload) {
  function __queryLabels({ host }) {
    const metadata = {
      entityFqdn: "wix.contacts.v4.label",
      method: "POST",
      methodFqn: "com.wixpress.contacts.labels.v4.ContactLabelsServiceV4.QueryLabels",
      packageName: PACKAGE_NAME,
      migrationOptions: {
        optInTransformResponse: true
      },
      url: resolveComWixpressContactsLabelsV4ContactLabelsServiceV4Url({
        protoPath: "/v4/labels/query",
        data: payload,
        host
      }),
      data: payload,
      transformResponse: (payload2) => transformPaths(payload2, [
        {
          transformFn: transformRESTTimestampToSDKTimestamp,
          paths: [
            { path: "labels.createdDate" },
            { path: "labels.updatedDate" }
          ]
        }
      ])
    };
    return metadata;
  }
  return __queryLabels;
}

// src/contacts-v4-label-labels.universal.ts
import { transformPaths as transformPaths2 } from "@wix/sdk-runtime/transformations/transform-paths";
import { createQueryUtils } from "@wix/sdk-runtime/query-builder-utils";
var LabelType = /* @__PURE__ */ ((LabelType2) => {
  LabelType2["SYSTEM"] = "SYSTEM";
  LabelType2["USER_DEFINED"] = "USER_DEFINED";
  LabelType2["WIX_APP_DEFINED"] = "WIX_APP_DEFINED";
  return LabelType2;
})(LabelType || {});
var SortOrder = /* @__PURE__ */ ((SortOrder2) => {
  SortOrder2["ASC"] = "ASC";
  SortOrder2["DESC"] = "DESC";
  return SortOrder2;
})(SortOrder || {});
var WebhookIdentityType = /* @__PURE__ */ ((WebhookIdentityType2) => {
  WebhookIdentityType2["UNKNOWN"] = "UNKNOWN";
  WebhookIdentityType2["ANONYMOUS_VISITOR"] = "ANONYMOUS_VISITOR";
  WebhookIdentityType2["MEMBER"] = "MEMBER";
  WebhookIdentityType2["WIX_USER"] = "WIX_USER";
  WebhookIdentityType2["APP"] = "APP";
  return WebhookIdentityType2;
})(WebhookIdentityType || {});
async function listLabels2(options) {
  const { httpClient, sideEffects } = arguments[1];
  const payload = renameKeysFromSDKRequestToRESTRequest({
    labelType: options?.labelType,
    namespace: options?.namespace,
    startsWith: options?.startsWith,
    sort: options?.sort,
    paging: options?.paging,
    language: options?.language
  });
  const reqOpts = listLabels(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
    return renameKeysFromRESTResponseToSDKResponse(result.data);
  } catch (err) {
    const transformedError = sdkTransformError(
      err,
      {
        spreadPathsToArguments: {},
        explicitPathsToArguments: {
          labelType: "$[0].labelType",
          namespace: "$[0].namespace",
          startsWith: "$[0].startsWith",
          sort: "$[0].sort",
          paging: "$[0].paging",
          language: "$[0].language"
        },
        singleArgumentUnchanged: false
      },
      ["options"]
    );
    sideEffects?.onError?.(err);
    throw transformedError;
  }
}
async function findOrCreateLabel2(displayName, options) {
  const { httpClient, sideEffects } = arguments[2];
  const payload = renameKeysFromSDKRequestToRESTRequest({
    displayName,
    language: options?.language
  });
  const reqOpts = findOrCreateLabel(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
    return renameKeysFromRESTResponseToSDKResponse(result.data);
  } catch (err) {
    const transformedError = sdkTransformError(
      err,
      {
        spreadPathsToArguments: {},
        explicitPathsToArguments: {
          displayName: "$[0]",
          language: "$[1].language"
        },
        singleArgumentUnchanged: false
      },
      ["displayName", "options"]
    );
    sideEffects?.onError?.(err);
    throw transformedError;
  }
}
async function getLabel2(key, options) {
  const { httpClient, sideEffects } = arguments[2];
  const payload = renameKeysFromSDKRequestToRESTRequest({
    key,
    language: options?.language
  });
  const reqOpts = getLabel(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
    return renameKeysFromRESTResponseToSDKResponse(result.data)?.label;
  } catch (err) {
    const transformedError = sdkTransformError(
      err,
      {
        spreadPathsToArguments: {},
        explicitPathsToArguments: { key: "$[0]", language: "$[1].language" },
        singleArgumentUnchanged: false
      },
      ["key", "options"]
    );
    sideEffects?.onError?.(err);
    throw transformedError;
  }
}
async function renameLabel(key, label, options) {
  const { httpClient, sideEffects } = arguments[3];
  const payload = renameKeysFromSDKRequestToRESTRequest({
    label: { ...label, key },
    language: options?.language
  });
  const reqOpts = updateLabel(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
    return renameKeysFromRESTResponseToSDKResponse(result.data)?.label;
  } catch (err) {
    const transformedError = sdkTransformError(
      err,
      {
        spreadPathsToArguments: { label: "$[1]" },
        explicitPathsToArguments: {
          "label.key": "$[0]",
          language: "$[2].language"
        },
        singleArgumentUnchanged: false
      },
      ["key", "label", "options"]
    );
    sideEffects?.onError?.(err);
    throw transformedError;
  }
}
async function deleteLabel2(key) {
  const { httpClient, sideEffects } = arguments[1];
  const payload = renameKeysFromSDKRequestToRESTRequest({ key });
  const reqOpts = deleteLabel(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
  } catch (err) {
    const transformedError = sdkTransformError(
      err,
      {
        spreadPathsToArguments: {},
        explicitPathsToArguments: { key: "$[0]" },
        singleArgumentUnchanged: false
      },
      ["key"]
    );
    sideEffects?.onError?.(err);
    throw transformedError;
  }
}
function queryLabels2(options) {
  const { httpClient, sideEffects } = arguments[1];
  return queryBuilder({
    func: async (payload) => {
      const reqOpts = queryLabels({
        ...payload,
        ...options ?? {}
      });
      sideEffects?.onSiteCall?.();
      try {
        const result = await httpClient.request(reqOpts);
        sideEffects?.onSuccess?.(result);
        return result;
      } catch (err) {
        sideEffects?.onError?.(err);
        throw err;
      }
    },
    requestTransformer: (query) => {
      const args = [query, options];
      return renameKeysFromSDKRequestToRESTRequest({
        ...args?.[1],
        query: args?.[0]
      });
    },
    responseTransformer: ({ data }) => {
      const transformedData = renameKeysFromRESTResponseToSDKResponse(
        transformPaths2(data, [])
      );
      return {
        items: transformedData?.labels,
        pagingMetadata: transformedData?.pagingMetadata
      };
    },
    errorTransformer: (err) => {
      const transformedError = sdkTransformError(err, {
        spreadPathsToArguments: {},
        explicitPathsToArguments: { query: "$[0]" },
        singleArgumentUnchanged: false
      });
      throw transformedError;
    },
    pagingMethod: "OFFSET",
    transformationPaths: {}
  });
}
async function typedQueryLabels(query, options) {
  const { httpClient, sideEffects } = arguments[2];
  const payload = renameKeysFromSDKRequestToRESTRequest({
    query,
    ...options
  });
  const reqOpts = queryLabels(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
    return renameKeysFromRESTResponseToSDKResponse(result.data);
  } catch (err) {
    const transformedError = sdkTransformError(
      err,
      {
        spreadPathsToArguments: {},
        explicitPathsToArguments: { query: "$[0]" },
        singleArgumentUnchanged: false
      },
      ["query", "options"]
    );
    sideEffects?.onError?.(err);
    throw transformedError;
  }
}
var utils = {
  query: /* @__PURE__ */ createQueryUtils()
};

// src/contacts-v4-label-labels.public.ts
function listLabels3(httpClient) {
  return (options) => listLabels2(
    options,
    // @ts-ignore
    { httpClient }
  );
}
function findOrCreateLabel3(httpClient) {
  return (displayName, options) => findOrCreateLabel2(
    displayName,
    options,
    // @ts-ignore
    { httpClient }
  );
}
function getLabel3(httpClient) {
  return (key, options) => getLabel2(
    key,
    options,
    // @ts-ignore
    { httpClient }
  );
}
function renameLabel2(httpClient) {
  return (key, label, options) => renameLabel(
    key,
    label,
    options,
    // @ts-ignore
    { httpClient }
  );
}
function deleteLabel3(httpClient) {
  return (key) => deleteLabel2(
    key,
    // @ts-ignore
    { httpClient }
  );
}
function queryLabels3(httpClient) {
  return (options) => queryLabels2(
    options,
    // @ts-ignore
    { httpClient }
  );
}
function typedQueryLabels2(httpClient) {
  return (query, options) => typedQueryLabels(
    query,
    options,
    // @ts-ignore
    { httpClient }
  );
}
var onLabelCreated = /* @__PURE__ */ EventDefinition(
  "wix.contacts.v4.label_created",
  true,
  (event) => renameKeysFromRESTResponseToSDKResponse2(
    transformPaths3(event, [
      {
        transformFn: transformRESTTimestampToSDKTimestamp2,
        paths: [
          { path: "entity.createdDate" },
          { path: "entity.updatedDate" },
          { path: "metadata.eventTime" }
        ]
      }
    ])
  )
)();
var onLabelDeleted = /* @__PURE__ */ EventDefinition(
  "wix.contacts.v4.label_deleted",
  true,
  (event) => renameKeysFromRESTResponseToSDKResponse2(
    transformPaths3(event, [
      {
        transformFn: transformRESTTimestampToSDKTimestamp2,
        paths: [
          { path: "undefined.createdDate" },
          { path: "undefined.updatedDate" },
          { path: "metadata.eventTime" }
        ]
      }
    ])
  )
)();
var onLabelUpdated = /* @__PURE__ */ EventDefinition(
  "wix.contacts.v4.label_updated",
  true,
  (event) => renameKeysFromRESTResponseToSDKResponse2(
    transformPaths3(event, [
      {
        transformFn: transformRESTTimestampToSDKTimestamp2,
        paths: [
          { path: "entity.createdDate" },
          { path: "entity.updatedDate" },
          { path: "metadata.eventTime" }
        ]
      }
    ])
  )
)();

// src/contacts-v4-label-labels.context.ts
import { createRESTModule } from "@wix/sdk-runtime/rest-modules";
import { createEventModule } from "@wix/sdk-runtime/event-definition-modules";
import { createQueryOverloadRouter } from "@wix/sdk-runtime/query-method-router";
function customQueryLabels(httpClient) {
  const router = /* @__PURE__ */ createQueryOverloadRouter({
    builderQueryFunction: (options) => queryLabels3(httpClient)(options),
    typedQueryFunction: (query, options) => typedQueryLabels2(httpClient)(query, options),
    hasOptionsParameter: true
  });
  function overloadedQuery(queryOrOptions, options) {
    return router(...arguments);
  }
  return overloadedQuery;
}
var listLabels4 = /* @__PURE__ */ createRESTModule(listLabels3);
var findOrCreateLabel4 = /* @__PURE__ */ createRESTModule(findOrCreateLabel3);
var getLabel4 = /* @__PURE__ */ createRESTModule(getLabel3);
var renameLabel3 = /* @__PURE__ */ createRESTModule(renameLabel2);
var deleteLabel4 = /* @__PURE__ */ createRESTModule(deleteLabel3);
var queryLabels4 = /* @__PURE__ */ createRESTModule(customQueryLabels);
var onLabelCreated2 = /* @__PURE__ */ createEventModule(onLabelCreated);
var onLabelDeleted2 = /* @__PURE__ */ createEventModule(onLabelDeleted);
var onLabelUpdated2 = /* @__PURE__ */ createEventModule(onLabelUpdated);
export {
  LabelType,
  SortOrder,
  WebhookIdentityType,
  deleteLabel4 as deleteLabel,
  findOrCreateLabel4 as findOrCreateLabel,
  getLabel4 as getLabel,
  listLabels4 as listLabels,
  onLabelCreated2 as onLabelCreated,
  onLabelDeleted2 as onLabelDeleted,
  onLabelUpdated2 as onLabelUpdated,
  queryLabels4 as queryLabels,
  renameLabel3 as renameLabel,
  utils
};
//# sourceMappingURL=index.mjs.map