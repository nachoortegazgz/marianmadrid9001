...
const getResponsiveImageProps = (imageProps, targetWidth, targetHeight) => {
  ...
  if (!widthProportion) {
    ...
    const baseUrl = baseHostURL || _max.STATIC_MEDIA_URL;
    const srcsetWithPrefix = srcset == null || (_srcset$dpr = srcset.dpr) == null ? void 0 : _srcset$dpr.map(s => /^[a-z]+:/.test(s) ? s : `${baseUrl}${s}`);
    ...
    const sources = getSources(widthProportion, width, height, targetHeight, targetWidth, uri, quality, name, alignType, hasAnimation, upscaleMethod, focalPoint, crop, encoding, siteMargin, displayMode, allowFullGIFTransformation, baseHostURL).filter(Boolean).reverse();
    return {
