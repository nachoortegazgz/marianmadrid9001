 ...
 * - Each service selection has either a `timeSlot` or a non-empty
 *   `rangeTimeSlots` (DAY-range path)
 * - Form submission filled
 *
...
function canBook(params) {
    ...
    return (serviceSelections.length > 0 &&
        serviceSelections.every((item) => item.timeSlot !== undefined ||
            (item.rangeTimeSlots !== undefined && item.rangeTimeSlots.length > 0)) &&
        formSubmission !== null);
}
