 ...
 *
 * **IMPORTANT**: This should wrap the entire application at BaseLayout level,
 * not individual pages. This ensures selections persist across navigation.
 *
 * @component
 * @example
 * ```tsx
...
function mapTimeSlotLocationToServiceLocation(timeSlotLocation) {
    ...
    // Convert to nested business/custom structure that CoreLocation expects
    const isCustomer = timeSlotLocation.locationType === auto_sdk_bookings_services_1.LocationTypeEnumLocationType.CUSTOMER;
    const isCustom = timeSlotLocation.locationType === auto_sdk_bookings_services_1.LocationTypeEnumLocationType.CUSTOM;
    // The two SDKs share BUSINESS/CUSTOM/CUSTOMER but the time-slots enum also
    // exposes UNKNOWN_LOCATION_TYPE, which V2Location.type does not accept.
    // Default to BUSINESS for any unrecognized value.
    const type = isCustomer
        ? auto_sdk_bookings_services_1.LocationTypeEnumLocationType.CUSTOMER
        : isCustom
            ? auto_sdk_bookings_services_1.LocationTypeEnumLocationType.CUSTOM
            : auto_sdk_bookings_services_1.LocationTypeEnumLocationType.BUSINESS;
    const location = {
...
function resolveLocation(bookingLocation, serviceSelections) {
    if (bookingLocation && bookingLocation.type === auto_sdk_bookings_services_1.LocationTypeEnumLocationType.BUSINESS) {
...
function PaymentData(props) {
    ...
    return children({ pricingServiceSelections });
}
