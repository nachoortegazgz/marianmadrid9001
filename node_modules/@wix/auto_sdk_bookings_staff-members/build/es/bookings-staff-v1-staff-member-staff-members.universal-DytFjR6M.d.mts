...
interface AssociatedWixIdentity {
     ...
     * @readonly
     * @deprecated Deprecated: Connection status.
     * Available only if you specify `ASSOCIATED_IDENTITY_STATUS` in the `fields` array.
...
/** @enumType */
type ConnectionStatusWithLiterals = ConnectionStatus | 'CONNECTED' | 'DISCONNECTED';
...
declare enum Gender {
    ...
    /** Female. */
    FEMALE = "FEMALE",
    /** Non-binary. Includes Germany's [diverse](https://www.gesetze-im-internet.de/englisch_sbgg/englisch_sbgg.html), Netherlands' [X](https://en.wikipedia.org/wiki/Legal_recognition_of_non-binary_gender#The_Netherlands), and similar terms. */
    NON_BINARY = "NON_BINARY"
}
/** @enumType */
type GenderWithLiterals = Gender | 'MALE' | 'FEMALE' | 'NON_BINARY';
...
/** Triggered when tags are modified on a staff member. */
interface TagsModified {
    /** Staff member whose tags were modified. */
    staffMember?: StaffMember;
...
interface SetServiceProviderResponse {
    /** Updated staff member. */
    staffMember?: StaffMember;
}
interface GetOrCreateStaffMemberRequest {
    /**
     * ID of the Wix user to get or create a connected staff member for.
     * @format GUID
     */
    wixUserId: string;
    /**
...
interface GetOrCreateStaffMemberResponse {
    /** Retrieved or created staff member. */
    staffMember?: StaffMember;
}
interface BulkGetOrCreateStaffMembersRequest {
    /**
     * IDs of the Wix users to get or create connected staff members for. If a `wixUserId` appears more
     * than once, the same staff member is returned at each of its positions.
     * @minSize 1
     * @maxSize 50
     * @format GUID
     */
    wixUserIds: string[];
    /**
     * Conditional fields to return on each staff member.
     * @maxSize 3
     */
    fields?: RequestedFieldsWithLiterals[];
}
interface BulkGetOrCreateStaffMembersResponse {
    /**
     * Per-user results, in the order of the requested `wixUserIds`.
     * @minSize 1
     * @maxSize 50
     */
    results?: BulkGetOrCreateStaffMemberResult[];
    /** Bulk action metadata (success/failure counts). */
...
interface BulkGetOrCreateStaffMemberResult {
    /** Metadata about the get-or-create for this Wix user (success/error, original index). */
    itemMetadata?: ItemMetadata;
    /** The retrieved or created staff member. Populated when the item succeeded. */
    staffMember?: StaffMember;
}
...
interface QueryStaffMembersV2Request {
    /** Information about filters, paging, and sorting. */
    query?: CursorQuery;
    /**
     * Conditional fields to return.
...
interface QueryStaffMembersV2Response {
    /** Retrieved staff members. */
    staffMembers?: StaffMember[];
    /** Paging metadata. */
    pagingMetadata?: CursorPagingMetadata;
}
interface SearchStaffMembersV2Request {
    /** Search criteria including filter, sort, and paging options. */
    search?: CursorSearch;
    /**
     * Conditional fields to return in the response.
...
interface SearchStaffMembersV2Response {
    /**
     * Retrieved staff members that match the search criteria specified in the request.
     *
     * Each staff member includes their profile information such as name, email, phone number,
...
interface CountStaffMembersV2Request {
    /**
     * Filter to base the count upon.
...
interface CountStaffMembersV2Response {
    /** Total number of staff members matching the given filter. */
    count?: number;
}
...
/** @enumType */
type DayWithLiterals = Day | 'UNDEFINED' | 'MON' | 'TUE' | 'WED' | 'THU' | 'FRI' | 'SAT' | 'SUN';
interface Frequency {
    /**
...
/** @enumType */
type TransparencyWithLiterals = Transparency | 'UNDEFINED' | 'FREE' | 'BUSY';
...
/** @enumType */
type LocationStatusWithLiterals = LocationStatus | 'ACTIVE' | 'INACTIVE';
/** For future use */
...
/** @enumType */
type LocationsLocationTypeWithLiterals = LocationsLocationType | 'UNKNOWN' | 'BRANCH' | 'OFFICES' | 'RECEPTION' | 'HEADQUARTERS' | 'INVENTORY';
...
/** @enumType */
type ApprovalStatusWithLiterals = ApprovalStatus | 'UNDEFINED' | 'PENDING' | 'APPROVED' | 'DECLINED';
...
/** @enumType */
type ScheduleStatusWithLiterals = ScheduleStatus | 'UNDEFINED' | 'CREATED' | 'CANCELLED';
interface Version {
    /** Schedule version number, updated each time the schedule is updated. */
...
/** @enumType */
type ConferenceTypeWithLiterals = ConferenceType | 'UNDEFINED' | 'ONLINE_MEETING_PROVIDER' | 'CUSTOM';
...
/** @enumType */
type StatusWithLiterals = Status | 'UNDEFINED' | 'CONFIRMED' | 'CANCELLED';
...
/** @enumType */
type SessionTypeWithLiterals = SessionType | 'UNDEFINED' | 'EVENT' | 'WORKING_HOURS' | 'TIME_AVAILABILITY' | 'AVAILABILITY';
...
/** @docsIgnore */
type ConnectStaffMemberToUserApplicationErrors = {
    code?: 'STAFF_INVITE_REQUIRES_ROLE';
    description?: string;
    data?: Record<string, any>;
};
/** @docsIgnore */
type BulkUpdateStaffMemberTagsApplicationErrors = {
...
interface StaffMemberConnectedToUserEnvelope {
 ...
 */
declare function onStaffMemberConnectedToUser(handler: (event: StaffMemberConnectedToUserEnvelope) => void | Promise<void>): void;
 ...
 * @slug created
 */
declare function onStaffMemberCreated(handler: (event: StaffMemberCreatedEnvelope) => void | Promise<void>): void;
 ...
 * @slug deleted
 */
declare function onStaffMemberDeleted(handler: (event: StaffMemberDeletedEnvelope) => void | Promise<void>): void;
interface StaffMemberDisconnectedFromUserEnvelope {
 ...
 */
declare function onStaffMemberDisconnectedFromUser(handler: (event: StaffMemberDisconnectedFromUserEnvelope) => void | Promise<void>): void;
interface StaffMemberFullyCreatedEnvelope {
    data: StaffMemberFullyCreated;
    metadata: EventMetadata;
}
/**
 * Triggered when a staff member is created and their working hours schedule is
 * attached to them. Fires after the staff member created event, and the payload
 * includes the `resource` field.
 * @permissionScope Manage_Wix_Rentals_App_Forms_submissions
 * @permissionScopeId SCOPE.RENTALS.MANAGE
 * @permissionScope Read Bookings - Public Data
 * @permissionScopeId SCOPE.DC-BOOKINGS.READ-BOOKINGS-PUBLIC
 ...
 * @slug fully_created
 */
declare function onStaffMemberFullyCreated(handler: (event: StaffMemberFullyCreatedEnvelope) => void | Promise<void>): void;
interface StaffMemberTagsModifiedEnvelope {
    data: TagsModified;
    metadata: EventMetadata;
}
/**
 * Triggered when a staff member's tags are added, removed, or changed.
 * @permissionScope Manage_Wix_Rentals_App_Forms_submissions
 * @permissionScopeId SCOPE.RENTALS.MANAGE
 * @permissionScope Read Bookings - Public Data
 * @permissionScopeId SCOPE.DC-BOOKINGS.READ-BOOKINGS-PUBLIC
 ...
 * @permissionId BOOKINGS.STAFF_MEMBER_READ
 * @webhook
 * @eventType wix.bookings.staff.v1.staff_member_tags_modified
 * @slug tags_modified
 */
declare function onStaffMemberTagsModified(handler: (event: StaffMemberTagsModifiedEnvelope) => void | Promise<void>): void;
 ...
 * @slug updated
 */
declare function onStaffMemberUpdated(handler: (event: StaffMemberUpdatedEnvelope) => void | Promise<void>): void;
/**
 ...
 */
declare function getStaffMember(staffMemberId: string, options?: GetStaffMemberOptions): Promise<NonNullablePaths<StaffMember, `resource.workingHoursSchedules` | `resource.workingHoursSchedules.${number}.shared` | `resource.usesDefaultWorkingHours` | `associatedWixIdentity.identificationData.anonymousVisitorId` | `associatedWixIdentity.identificationData.memberId` | `associatedWixIdentity.identificationData.wixUserId` | `associatedWixIdentity.identificationData.appId` | `associatedWixIdentity.connectionStatus` | `associatedWixIdentity.connection.status` | `tags.privateTags.tagIds`, 3>>;
 ...
 * @param staffMemberId - ID of the staff member to delete permanently.
 * @public
 * @requiredField staffMemberId
 * @permissionId BOOKINGS.STAFF_MEMBER_REMOVE_FROM_TRASH_BIN
 * @applicableIdentity APP
 * @fqn wix.bookings.staff.v1.StaffMembersService.RemoveStaffMemberFromTrashBin
 */
declare function removeStaffMemberFromTrashBin(staffMemberId: string): Promise<void>;
/**
 ...
 */
declare function updateStaffMember(_id: string, staffMember: NonNullablePaths<UpdateStaffMember, `revision`, 0>, options?: UpdateStaffMemberOptions): Promise<NonNullablePaths<StaffMember, `resource.workingHoursSchedules` | `resource.workingHoursSchedules.${number}.shared` | `resource.usesDefaultWorkingHours` | `associatedWixIdentity.identificationData.anonymousVisitorId` | `associatedWixIdentity.identificationData.memberId` | `associatedWixIdentity.identificationData.wixUserId` | `associatedWixIdentity.identificationData.appId` | `associatedWixIdentity.connectionStatus` | `associatedWixIdentity.connection.status` | `tags.privateTags.tagIds`, 3>>;
 ...
 */
declare function deleteStaffMember(staffMemberId: string): Promise<void>;
/**
 ...
 */
declare function queryStaffMembers(options?: QueryStaffMembersOptions): StaffMembersQueryBuilder;
...
interface StaffMembersQueryBuilder {
     ...
     * @param value - Value to compare against.
     */
    ge: (propertyName: '_id' | 'description' | 'resourceId' | 'resource.id' | 'associatedWixIdentity.identificationData.wixUserId' | 'associatedWixIdentity.identificationData.contactId' | '_createdDate' | '_updatedDate', value: any) => StaffMembersQueryBuilder;
    /** @param propertyName - Property whose value is compared with `value`.
     * @param value - Value to compare against.
     */
    gt: (propertyName: '_id' | 'description' | 'resourceId' | 'resource.id' | 'associatedWixIdentity.identificationData.wixUserId' | 'associatedWixIdentity.identificationData.contactId' | '_createdDate' | '_updatedDate', value: any) => StaffMembersQueryBuilder;
    /** @param propertyName - Property whose value is compared with `value`.
     * @param value - Value to compare against.
     */
    le: (propertyName: '_id' | 'description' | 'resourceId' | 'resource.id' | 'associatedWixIdentity.identificationData.wixUserId' | 'associatedWixIdentity.identificationData.contactId' | '_createdDate' | '_updatedDate', value: any) => StaffMembersQueryBuilder;
    /** @param propertyName - Property whose value is compared with `value`.
     * @param value - Value to compare against.
     */
    lt: (propertyName: '_id' | 'description' | 'resourceId' | 'resource.id' | 'associatedWixIdentity.identificationData.wixUserId' | 'associatedWixIdentity.identificationData.contactId' | '_createdDate' | '_updatedDate', value: any) => StaffMembersQueryBuilder;
     ...
     */
    hasSome: (propertyName: string, value: any[]) => StaffMembersQueryBuilder;
    /** @param propertyName - Property whose value is compared with `values`.
     * @param values - List of values to compare against.
     */
...
declare const utils: {
    query: _wix_sdk_types.QueryHelpers<StaffMember, StaffMemberQuerySpec, StaffMemberQuery>;
    search: _wix_sdk_types.SearchHelpers<StaffMember, StaffMemberSearchSpec, StaffMemberSearch>;
};
/**
 ...
 */
declare function countStaffMembers(options?: CountStaffMembersOptions): Promise<NonNullablePaths<CountStaffMembersResponse, `count`, 0>>;
...
declare function connectStaffMemberToUser(staffMemberId: string, options?: ConnectStaffMemberToUserOptions): Promise<NonNullablePaths<ConnectStaffMemberToUserResponse, `staffMember.resource.workingHoursSchedules` | `staffMember.resource.workingHoursSchedules.${number}.shared` | `staffMember.resource.usesDefaultWorkingHours` | `staffMember.associatedWixIdentity.identificationData.anonymousVisitorId` | `staffMember.associatedWixIdentity.identificationData.memberId` | `staffMember.associatedWixIdentity.identificationData.wixUserId` | `staffMember.associatedWixIdentity.identificationData.appId` | `staffMember.associatedWixIdentity.connectionStatus` | `staffMember.associatedWixIdentity.connection.status` | `staffMember.tags.privateTags.tagIds`, 4> & {
    __applicationErrorsType?: ConnectStaffMemberToUserApplicationErrors;
}>;
 ...
 * @param staffMemberId - ID of the staff member whose service provider status to set.
 * @param serviceProvider - Whether the staff member should be a service provider. When `true`, the staff
 * member appears in booking flows, and the Wix user connected to them is granted
 * the Staff role if they don't already have it.
 *
 * When `false`, the staff member no longer appears in booking flows. Their working
 ...
 * @requiredField serviceProvider
 * @requiredField staffMemberId
 * @permissionId bookings:staff:v1:staff_member:set_service_provider
 * @applicableIdentity APP
 * @fqn wix.bookings.staff.v1.StaffMembersService.SetServiceProvider
 */
declare function setServiceProvider(staffMemberId: string, serviceProvider: boolean, options?: SetServiceProviderOptions): Promise<NonNullablePaths<SetServiceProviderResponse, `staffMember.resource.workingHoursSchedules` | `staffMember.resource.workingHoursSchedules.${number}.shared` | `staffMember.resource.usesDefaultWorkingHours` | `staffMember.associatedWixIdentity.identificationData.anonymousVisitorId` | `staffMember.associatedWixIdentity.identificationData.memberId` | `staffMember.associatedWixIdentity.identificationData.wixUserId` | `staffMember.associatedWixIdentity.identificationData.appId` | `staffMember.associatedWixIdentity.connectionStatus` | `staffMember.associatedWixIdentity.connection.status` | `staffMember.tags.privateTags.tagIds`, 4>>;
interface SetServiceProviderOptions {
    /**
     * Conditional fields to return.
     * @maxSize 3
     */
...
interface GetOrCreateStaffMemberOptions {
    /**
     * Conditional fields to return.
     * @maxSize 3
     */
...
interface BulkGetOrCreateStaffMembersOptions {
    /**
     * Conditional fields to return on each staff member.
     * @maxSize 3
     */
...
interface StaffMemberSearchSpec extends SearchSpec {
    searchable: ['name'];
    paging: 'cursor';
    wql: [
        {
        ...
        },
        {
            ...
            fields: [
                'resource.locationOptions.specificLocationOptions.businessLocations'
            ];
            sort: 'NONE';
        }
    ];
}
type CommonSearchWithEntityContext = Search<StaffMember, StaffMemberSearchSpec>;
 ...
 * @public
 * @documentationMaturity preview
 * @permissionId bookings:staff:v1:staff_member:read
 * @applicableIdentity APP
 * @fqn wix.bookings.staff.v1.StaffMembersService.QueryStaffMembersV2
 */
declare function queryStaffMembersV2(options?: QueryStaffMembersV2Options): Promise<NonNullablePaths<QueryStaffMembersV2Response, `staffMembers` | `staffMembers.${number}.resource.usesDefaultWorkingHours` | `staffMembers.${number}.associatedWixIdentity.identificationData.anonymousVisitorId` | `staffMembers.${number}.associatedWixIdentity.identificationData.memberId` | `staffMembers.${number}.associatedWixIdentity.identificationData.wixUserId` | `staffMembers.${number}.associatedWixIdentity.identificationData.appId` | `staffMembers.${number}.associatedWixIdentity.connectionStatus` | `staffMembers.${number}.associatedWixIdentity.connection.status`, 4>>;
interface QueryStaffMembersV2Options {
    /** Information about filters, paging, and sorting. */
    query?: CursorQuery;
    /**
     * Conditional fields to return.
 ...
 * @permissionId bookings:staff:v1:staff_member:read
 * @applicableIdentity APP
 * @fqn wix.bookings.staff.v1.StaffMembersService.SearchStaffMembersV2
 */
declare function searchStaffMembersV2(search: CursorSearch, options?: SearchStaffMembersV2Options): Promise<NonNullablePaths<SearchStaffMembersV2Response, `staffMembers` | `staffMembers.${number}.resource.usesDefaultWorkingHours` | `staffMembers.${number}.associatedWixIdentity.identificationData.anonymousVisitorId` | `staffMembers.${number}.associatedWixIdentity.identificationData.memberId` | `staffMembers.${number}.associatedWixIdentity.identificationData.wixUserId` | `staffMembers.${number}.associatedWixIdentity.identificationData.appId` | `staffMembers.${number}.associatedWixIdentity.connectionStatus` | `staffMembers.${number}.associatedWixIdentity.connection.status` | `aggregationData.results` | `aggregationData.results.${number}.scalar.type` | `aggregationData.results.${number}.scalar.value` | `aggregationData.results.${number}.name` | `aggregationData.results.${number}.type` | `aggregationData.results.${number}.fieldPath`, 4>>;
interface SearchStaffMembersV2Options {
    /**
     * Conditional fields to return in the response.
     * @maxSize 3
     */
 ...
 * @permissionId bookings:staff:v1:staff_member:read
 * @applicableIdentity APP
 * @fqn wix.bookings.staff.v1.StaffMembersService.CountStaffMembersV2
 */
declare function countStaffMembersV2(options?: CountStaffMembersV2Options): Promise<NonNullablePaths<CountStaffMembersV2Response, `count`, 0>>;
interface CountStaffMembersV2Options {
    /**
     * Filter to base the count upon.
     * See [Filtering and Sorting](https://dev.wix.com/docs/api-reference/business-solutions/bookings/staff-members/staff-members/filtering-and-sorting)
     * for a complete list of supported filters.
 ...
 */
declare function disconnectStaffMemberFromUser(staffMemberId: string, options?: DisconnectStaffMemberFromUserOptions): Promise<NonNullablePaths<DisconnectStaffMemberFromUserResponse, `staffMember.resource.workingHoursSchedules` | `staffMember.resource.workingHoursSchedules.${number}.shared` | `staffMember.resource.usesDefaultWorkingHours` | `staffMember.associatedWixIdentity.identificationData.anonymousVisitorId` | `staffMember.associatedWixIdentity.identificationData.memberId` | `staffMember.associatedWixIdentity.identificationData.wixUserId` | `staffMember.associatedWixIdentity.identificationData.appId` | `staffMember.associatedWixIdentity.connectionStatus` | `staffMember.associatedWixIdentity.connection.status` | `staffMember.tags.privateTags.tagIds`, 4>>;
 ...
 */
declare function assignWorkingHoursSchedule(staffMemberId: string, scheduleId: string, options?: AssignWorkingHoursScheduleOptions): Promise<NonNullablePaths<AssignWorkingHoursScheduleResponse, `staffMember.resource.workingHoursSchedules` | `staffMember.resource.workingHoursSchedules.${number}.shared` | `staffMember.resource.usesDefaultWorkingHours` | `staffMember.associatedWixIdentity.identificationData.anonymousVisitorId` | `staffMember.associatedWixIdentity.identificationData.memberId` | `staffMember.associatedWixIdentity.identificationData.wixUserId` | `staffMember.associatedWixIdentity.identificationData.appId` | `staffMember.associatedWixIdentity.connectionStatus` | `staffMember.associatedWixIdentity.connection.status` | `staffMember.tags.privateTags.tagIds`, 4>>;
 ...
 */
declare function assignCustomSchedule(staffMemberId: string, scheduleId: string, options?: AssignCustomScheduleOptions): Promise<NonNullablePaths<AssignCustomScheduleResponse, `staffMember.resource.workingHoursSchedules` | `staffMember.resource.workingHoursSchedules.${number}.shared` | `staffMember.resource.usesDefaultWorkingHours` | `staffMember.associatedWixIdentity.identificationData.anonymousVisitorId` | `staffMember.associatedWixIdentity.identificationData.memberId` | `staffMember.associatedWixIdentity.identificationData.wixUserId` | `staffMember.associatedWixIdentity.identificationData.appId` | `staffMember.associatedWixIdentity.connectionStatus` | `staffMember.associatedWixIdentity.connection.status` | `staffMember.tags.privateTags.tagIds`, 4>>;
 ...
 * @param filter - Filter to base the update upon.
 * See [Query Staff Members](https://dev.wix.com/docs/api-reference/business-solutions/bookings/staff-members/staff-members/query-staff-members)
 * for a complete list of supported filters.
 * @public
 ...
 */
declare function bulkUpdateStaffMemberTagsByFilter(filter: Record<string, any>, options?: BulkUpdateStaffMemberTagsByFilterOptions): Promise<NonNullablePaths<BulkUpdateStaffMemberTagsByFilterResponse, `jobId`, 0> & {
    __applicationErrorsType?: BulkUpdateStaffMemberTagsByFilterApplicationErrors;
}>;
interface BulkUpdateStaffMemberTagsByFilterOptions {
    /** Tags to add to the staff members. */
    assignTags?: Tags;
    /** Tags to remove from the staff members. */
    unassignTags?: Tags;
...
export { RequestedFields as $, type AssignWorkingHoursScheduleOptions as A, type BulkUpdateStaffMemberTagsOptions as B, type CreateStaffMemberOptions as C, type DisconnectStaffMemberFromUserOptions as D, type BulkUpdateStaffMemberTagsByFilterOptions as E, type BulkUpdateStaffMemberTagsByFilterResponse as F, type GetStaffMemberOptions as G, type BulkUpdateStaffMemberTagsByFilterApplicationErrors as H, type StaffMemberConnectedToUserEnvelope as I, type StaffMemberCreatedEnvelope as J, type StaffMemberDeletedEnvelope as K, type ListDeletedStaffMembersOptions as L, type StaffMemberDisconnectedFromUserEnvelope as M, type StaffMemberFullyCreatedEnvelope as N, type StaffMemberTagsModifiedEnvelope as O, type StaffMemberUpdatedEnvelope as P, type QueryStaffMembersV2Options as Q, type StaffMemberQuery as R, type StaffMember as S, type QueryStaffMembersOptions as T, type UpdateStaffMember as U, typedQueryStaffMembers as V, type StaffMembersQueryBuilder as W, IdentityType as X, AssociatedWixIdentityConnectionStatusEnumConnectionStatus as Y, ConnectionStatus as Z, Gender as _, type GetDeletedStaffMemberOptions as a, type Sorting as a$, SortOrder as a0, Mode as a1, ScalarType as a2, AggregationType as a3, Day as a4, Transparency as a5, RecurringIntervalType as a6, LocationType as a7, LocationStatus as a8, LocationsLocationType as a9, type ExtendedFields as aA, type Tags as aB, type TagList as aC, type StaffMemberDisconnectedFromUser as aD, type StaffMemberConnectedToUser as aE, type StaffMemberFullyCreated as aF, type TagsModified as aG, type CreateStaffMemberRequest as aH, type CreateStaffMemberResponse as aI, type GetStaffMemberRequest as aJ, type GetStaffMemberResponse as aK, type GetDeletedStaffMemberRequest as aL, type ListDeletedStaffMembersRequest as aM, type CursorPaging as aN, type CursorPagingMetadata as aO, type Cursors as aP, type RemoveStaffMemberFromTrashBinRequest as aQ, type RemoveStaffMemberFromTrashBinResponse as aR, type RestoreStaffMemberFromTrashBinRequest as aS, type RestoreStaffMemberFromTrashBinResponse as aT, type UpdateStaffMemberRequest as aU, type UpdateStaffMemberResponse as aV, type DeleteStaffMemberRequest as aW, type DeleteStaffMemberResponse as aX, type QueryStaffMembersRequest as aY, type CursorQuery as aZ, type CursorQueryPagingMethodOneOf as a_, DayOfWeek as aa, ApprovalStatus as ab, ScheduleStatus as ac, ConferenceType as ad, CalendarType as ae, Status as af, SessionType as ag, WebhookIdentityType as ah, type MediaItem as ai, type MediaItemMediaOneOf as aj, type Resource as ak, type WorkingHoursSchedule as al, type EventSchedule as am, type LocationOptions as an, type SpecificLocation as ao, type BusinessLocation as ap, type AssociatedWixIdentity as aq, type CommonIdentificationData as ar, type CommonIdentificationDataIdOneOf as as, type Connection as at, type AssociatedConferencingProviders as au, type AssociatedConferencingProvider as av, type AssociatedConferencingAccounts as aw, type AssociatedConferencingAccount as ax, type AssociatedConferencingAccountAccountOneOf as ay, type CustomConferenceAccount as az, type GetDeletedStaffMemberResponse as b, type RecurringInterval as b$, type QueryStaffMembersResponse as b0, type QueryStaffMembersMultiLanguageRequest as b1, type QueryStaffMembersMultiLanguageResponse as b2, type CountStaffMembersRequest as b3, type ConnectStaffMemberToUserRequest as b4, type SetServiceProviderRequest as b5, type GetOrCreateStaffMemberRequest as b6, type GetOrCreateStaffMemberResponse as b7, type BulkGetOrCreateStaffMembersRequest as b8, type BulkGetOrCreateStaffMembersResponse as b9, type AggregationResults as bA, type AggregationResultsResultOneOf as bB, type QueryStaffMembersV2Request as bC, type SearchStaffMembersV2Request as bD, type CountStaffMembersV2Request as bE, type DisconnectStaffMemberFromUserRequest as bF, type AssignWorkingHoursScheduleRequest as bG, type AssignCustomScheduleRequest as bH, type BulkUpdateStaffMemberTagsRequest as bI, type BulkUpdateStaffMemberTagsResult as bJ, type BulkUpdateStaffMemberTagsByFilterRequest as bK, type RestoreStaffRequest as bL, type RestoreStaffResponse as bM, type Empty as bN, type PolicyRemovedFromContributor as bO, type PolicyUpdatedForContributor as bP, type DomainEvent as bQ, type DomainEventBodyOneOf as bR, type EntityCreatedEvent as bS, type RestoreInfo as bT, type EntityUpdatedEvent as bU, type EntityDeletedEvent as bV, type ActionEvent as bW, type ScheduleNotification as bX, type ScheduleNotificationEventOneOf as bY, type ScheduleCreated as bZ, type Schedule as b_, type ItemMetadata as ba, type ApplicationError as bb, type BulkGetOrCreateStaffMemberResult as bc, type BulkActionMetadata as bd, type SearchStaffMembersRequest as be, type CursorSearchPagingMethodOneOf as bf, type SearchDetails as bg, type AggregationData as bh, type ValueAggregationResult as bi, type RangeAggregationResult as bj, type NestedAggregationResults as bk, type NestedAggregationResultsResultOneOf as bl, type ValueResults as bm, type RangeResults as bn, type AggregationResultsScalarResult as bo, type NestedValueAggregationResult as bp, type ValueResult as bq, type RangeResult as br, type ScalarResult as bs, type NestedResultValue as bt, type NestedResultValueResultOneOf as bu, type Results as bv, type DateHistogramResult as bw, type GroupByValueResults as bx, type DateHistogramResults as by, type NestedResults as bz, type ListDeletedStaffMembersResponse as c, type AssociatedWixIdentityConnectionStatusEnumConnectionStatusWithLiterals as c$, type Interval as c0, type Frequency as c1, type LinkedSchedule as c2, type Location as c3, type Address as c4, type AddressStreetOneOf as c5, type StreetAddress as c6, type AddressLocation as c7, type Subdivision as c8, type LocationsLocation as c9, type SessionUpdated as cA, type SessionCancelled as cB, type AvailabilityPolicyUpdated as cC, type AvailabilityPolicy as cD, type IntervalSplit as cE, type RecurringSessionSplit as cF, type ScheduleUnassignedFromUser as cG, type MultipleSessionsCreated as cH, type ScheduleWithSessions as cI, type SitePropertiesOnScheduleCreation as cJ, type MigrationEvent as cK, type MigrationData as cL, type StaffData as cM, type MessageEnvelope as cN, type IdentificationData as cO, type IdentificationDataIdOneOf as cP, type AccountInfo as cQ, type BaseEventMetadata as cR, type EventMetadata as cS, type AccountInfoMetadata as cT, type StaffMembersQueryResult as cU, type StaffMemberQuerySpec as cV, type GetOrCreateStaffMemberOptions as cW, type BulkGetOrCreateStaffMembersOptions as cX, type StaffMemberSearchSpec as cY, utils as cZ, type IdentityTypeWithLiterals as c_, type LocationsAddress as ca, type LocationsStreetAddress as cb, type LocationsAddressLocation as cc, type BusinessSchedule as cd, type TimePeriod as ce, type SpecialHourPeriod as cf, type Rate as cg, type Price as ch, type Availability as ci, type AvailabilityConstraints as cj, type SplitInterval as ck, type Participant as cl, type ExternalCalendarOverrides as cm, type Version as cn, type ConferenceProvider as co, type CalendarConference as cp, type ScheduleUpdated as cq, type RecurringSessionsUpdated as cr, type Session as cs, type CalendarDateTime as ct, type LocalDateTime as cu, type ExternalCalendarInfo as cv, type SessionVersion as cw, type ParticipantNotification as cx, type ScheduleCancelled as cy, type SessionCreated as cz, type UpdateStaffMemberOptions as d, type ConnectionStatusWithLiterals as d0, type GenderWithLiterals as d1, type RequestedFieldsWithLiterals as d2, type SortOrderWithLiterals as d3, type ModeWithLiterals as d4, type ScalarTypeWithLiterals as d5, type AggregationTypeWithLiterals as d6, type DayWithLiterals as d7, type TransparencyWithLiterals as d8, type RecurringIntervalTypeWithLiterals as d9, updateStaffMember as dA, deleteStaffMember as dB, queryStaffMembers as dC, countStaffMembers as dD, connectStaffMemberToUser as dE, setServiceProvider as dF, queryStaffMembersV2 as dG, searchStaffMembersV2 as dH, countStaffMembersV2 as dI, disconnectStaffMemberFromUser as dJ, assignWorkingHoursSchedule as dK, assignCustomSchedule as dL, bulkUpdateStaffMemberTags as dM, bulkUpdateStaffMemberTagsByFilter as dN, type LocationTypeWithLiterals as da, type LocationStatusWithLiterals as db, type LocationsLocationTypeWithLiterals as dc, type DayOfWeekWithLiterals as dd, type ApprovalStatusWithLiterals as de, type ScheduleStatusWithLiterals as df, type ConferenceTypeWithLiterals as dg, type CalendarTypeWithLiterals as dh, type StatusWithLiterals as di, type SessionTypeWithLiterals as dj, type WebhookIdentityTypeWithLiterals as dk, type CommonQueryWithEntityContext as dl, type CommonSearchWithEntityContext as dm, onStaffMemberConnectedToUser as dn, onStaffMemberCreated as dp, onStaffMemberDeleted as dq, onStaffMemberDisconnectedFromUser as dr, onStaffMemberFullyCreated as ds, onStaffMemberTagsModified as dt, onStaffMemberUpdated as du, createStaffMember as dv, getStaffMember as dw, getDeletedStaffMember as dx, listDeletedStaffMembers as dy, removeStaffMemberFromTrashBin as dz, type CountStaffMembersOptions as e, type CountStaffMembersResponse as f, type ConnectStaffMemberToUserOptions as g, type ConnectStaffMemberToUserResponse as h, type ConnectStaffMemberToUserApplicationErrors as i, type SetServiceProviderOptions as j, type SetServiceProviderResponse as k, type StaffMemberSearch as l, type SearchStaffMembersOptions as m, type SearchStaffMembersResponse as n, type QueryStaffMembersV2Response as o, type CursorSearch as p, type SearchStaffMembersV2Options as q, type SearchStaffMembersV2Response as r, type CountStaffMembersV2Options as s, type CountStaffMembersV2Response as t, type DisconnectStaffMemberFromUserResponse as u, type AssignWorkingHoursScheduleResponse as v, type AssignCustomScheduleOptions as w, type AssignCustomScheduleResponse as x, type BulkUpdateStaffMemberTagsResponse as y, type BulkUpdateStaffMemberTagsApplicationErrors as z };
