"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);

// index.ts
var index_exports = {};
__export(index_exports, {
  IdentityType: () => IdentityType,
  LocationType: () => LocationType,
  Mode: () => Mode,
  RequestedGender: () => RequestedGender,
  UnitType: () => UnitType,
  provideHandlers: () => provideHandlers2,
  publicProvideHandlers: () => provideHandlers
});
module.exports = __toCommonJS(index_exports);

// src/service-plugins-types.ts
var RequestedGender = /* @__PURE__ */ ((RequestedGender2) => {
  RequestedGender2["ANY_MALE"] = "ANY_MALE";
  RequestedGender2["ANY_FEMALE"] = "ANY_FEMALE";
  return RequestedGender2;
})(RequestedGender || {});
var LocationType = /* @__PURE__ */ ((LocationType2) => {
  LocationType2["CUSTOM"] = "CUSTOM";
  LocationType2["BUSINESS"] = "BUSINESS";
  LocationType2["CUSTOMER"] = "CUSTOMER";
  return LocationType2;
})(LocationType || {});
var UnitType = /* @__PURE__ */ ((UnitType2) => {
  UnitType2["HOUR"] = "HOUR";
  UnitType2["DAY"] = "DAY";
  return UnitType2;
})(UnitType || {});
var Mode = /* @__PURE__ */ ((Mode2) => {
  Mode2["INTERSECT_RESOURCE_HOURS"] = "INTERSECT_RESOURCE_HOURS";
  Mode2["OVERRIDE_RESOURCE_HOURS"] = "OVERRIDE_RESOURCE_HOURS";
  return Mode2;
})(Mode || {});
var IdentityType = /* @__PURE__ */ ((IdentityType2) => {
  IdentityType2["UNKNOWN"] = "UNKNOWN";
  IdentityType2["ANONYMOUS_VISITOR"] = "ANONYMOUS_VISITOR";
  IdentityType2["MEMBER"] = "MEMBER";
  IdentityType2["WIX_USER"] = "WIX_USER";
  IdentityType2["APP"] = "APP";
  return IdentityType2;
})(IdentityType || {});

// src/interfaces-bookings-availability-v2-availability-time-slots-configuration-provider.public.ts
var import_sdk_types = require("@wix/sdk-types");
var import_rename_all_nested_keys = require("@wix/sdk-runtime/rename-all-nested-keys");
var provideHandlers = (0, import_sdk_types.ServicePluginDefinition)("AVAILABILITY_TIME_SLOTS_CONFIGURATION_PROVIDER", [
  {
    name: "listAvailabilityTimeSlotConfigurations",
    primaryHttpMappingPath: "/v2/list-availability-time-slot-configurations",
    transformations: {
      toREST: (payload) => {
        const toRestResponse = payload;
        return (0, import_rename_all_nested_keys.renameKeysFromSDKRequestToRESTRequest)(toRestResponse);
      },
      fromREST: (payload) => {
        const fromRestRequest = payload;
        return (0, import_rename_all_nested_keys.renameKeysFromRESTResponseToSDKResponse)(fromRestRequest);
      }
    }
  }
]);

// src/interfaces-bookings-availability-v2-availability-time-slots-configuration-provider.context.ts
var import_service_plugin_modules = require("@wix/sdk-runtime/service-plugin-modules");
var provideHandlers2 = /* @__PURE__ */ (0, import_service_plugin_modules.createServicePluginModule)(
  provideHandlers
);
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  IdentityType,
  LocationType,
  Mode,
  RequestedGender,
  UnitType,
  provideHandlers,
  publicProvideHandlers
});
//# sourceMappingURL=index.js.map