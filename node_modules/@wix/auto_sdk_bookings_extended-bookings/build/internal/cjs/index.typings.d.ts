...
interface ExtendedBooking {
    ...
    /**
     * Event notes associated with the booking's event.
     * Available only when specified `withEventNotes` as `true`.
     *
     * You must have the `CALENDAR.EVENT_READ_PI` permission scope
     * when passing `true`.
     * @internal
     */
    eventNotes?: string | null;
    /**
     * Deferred charge info. Available only when `withDeferredCharge` is `true`.
     * @internal
     */
    deferredChargeInfo?: DeferredChargeInfo;
}
declare enum DeferredChargeStatus {
    /** Charge is scheduled and waiting to be initiated. */
    PENDING = "PENDING",
    /** Charge has been submitted to the payment provider. */
    INITIATED = "INITIATED",
    /** Charge was successfully collected. */
    CHARGED = "CHARGED",
    /** Charge was declined by the payment provider. */
    DECLINED = "DECLINED",
    /** A retry is scheduled after a transient decline. */
    RETRYING = "RETRYING",
    /** Charge was skipped (e.g., booking has no payment method on file). */
    SKIPPED = "SKIPPED",
    /** Charge was canceled before collection. */
    CANCELED = "CANCELED",
    /** Charge was permanently abandoned after exhausting all retries. */
    ABANDONED = "ABANDONED"
}
/** @enumType */
type DeferredChargeStatusWithLiterals = DeferredChargeStatus | 'PENDING' | 'INITIATED' | 'CHARGED' | 'DECLINED' | 'RETRYING' | 'SKIPPED' | 'CANCELED' | 'ABANDONED';
declare enum DeferredChargeDeclineReason {
    /** The payment method had insufficient funds. */
    INSUFFICIENT_FUNDS = "INSUFFICIENT_FUNDS",
    /** The payment method has expired. */
    EXPIRED = "EXPIRED",
    /** The charge attempt failed for a general reason. */
    CHARGE_FAILED = "CHARGE_FAILED",
    /** The charge failed for another reason. */
    OTHER = "OTHER"
}
/** @enumType */
type DeferredChargeDeclineReasonWithLiterals = DeferredChargeDeclineReason | 'INSUFFICIENT_FUNDS' | 'EXPIRED' | 'CHARGE_FAILED' | 'OTHER';
...
interface Order {
    ...
    bookingPriceDescription?: PriceDescription;
    ...
    /**
     * Booking line item ID associated with this order.
     * @internal
     * @minLength 1
     * @maxLength 50
     */
    ...
    /**
     * Indicates the order only contains a single line item.
     * @internal
     */
    singleItemOrder?: boolean | null;
    /**
    ...
    _updatedDate?: Date | null;
    /**
     * Additional fees applied to the booking line item.
     * @internal
     * @maxSize 100
     */
    additionalFees?: AdditionalFee[];
    /**
     * ID of the location associated with the order.
     * @internal
     * @format GUID
     */
    locationId?: string | null;
    /**
     * Coupon applied to the order, when the underlying eCommerce order includes a coupon discount.
     * @internal
     */
...
/** @enumType */
type PaymentStatusEnumPaymentStatusWithLiterals = PaymentStatusEnumPaymentStatus | 'NOT_PAID' | 'PAID' | 'PARTIALLY_REFUNDED' | 'FULLY_REFUNDED' | 'PENDING' | 'PARTIALLY_PAID' | 'PENDING_MERCHANT' | 'CANCELED' | 'DECLINED';
interface Price {
    /**
     * Amount.
...
/** Coupon details exposed for an eCommerce order (from the order's applied discounts). */
interface CouponInfo {
    ...
    /** Coupon display name. */
    name?: string;
    /** Discount amount attributed to the coupon. */
    amount?: Price;
}
interface Waiver {
    /**
...
interface OrderTransactions {
    ...
    chargedMembershipPaymentDetails?: MembershipPaymentDetails;
    /**
     * Sum of refund transactions amounts made to the buyer.
     * @decimalValue options { gte:0, lte:1000000000000000, maxScale:2 }
     */
    refundSumAmounts?: string | null;
}
interface MembershipPaymentDetails {
    ...
    /**
     * The amount of credits redeemed from the membership for this booking.
     * @internal
     * @decimalValue options { gte:0, maxScale:4 }
     * @readonly
     */
    redeemCost?: string | null;
}
...
/** Information about a booking's subscription payment plan. Populated for course bookings with subscription pricing only. */
interface SubscriptionInfo {
     ...
     */
    billingFrequency?: FrequencyWithLiterals;
    /**
    ...
    /**
     * Date the subscription's cancellation takes effect.
     *
     * Returned only when a cancellation is scheduled, typically for the end of the current billing cycle.
     * @internal
     */
    cancellationDate?: Date | null;
    /**
     * Date the subscription's next billing cycle begins.
     *
     * Returned when the subscription has an upcoming billing cycle.
     * For an active subscription, this is the date an end-of-billing-period cancellation would take effect.
     * @internal
     */
...
/** @enumType */
type FrequencyWithLiterals = Frequency | 'MONTHLY';
declare enum SubscriptionStatus {
    /** Subscription created but not yet paid for. Transitions to `PENDING` or `ACTIVE` after the first successful payment. */
    DRAFT = "DRAFT",
    /** Subscription is paid but hasn't started yet. The start date is in the future. */
    PENDING = "PENDING",
    /** Subscription is active and billing. */
    ACTIVE = "ACTIVE",
    /** Subscription is temporarily paused. */
    PAUSED = "PAUSED",
    /** Subscription was canceled before its scheduled end. */
    CANCELED = "CANCELED",
    /** Subscription completed all billing cycles. */
    ENDED = "ENDED"
}
/** @enumType */
type SubscriptionStatusWithLiterals = SubscriptionStatus | 'DRAFT' | 'PENDING' | 'ACTIVE' | 'PAUSED' | 'CANCELED' | 'ENDED';
...
interface DeferredChargeInfo extends DeferredChargeInfoStatusDetailsOneOf {
    /** Reason the charge was declined. */
    declinedOptions?: DeferredChargeDeclineReasonWithLiterals;
    /** Current status of the deferred charge. */
    status?: DeferredChargeStatusWithLiterals;
    /**
     * ID of the deferred charge.
     *
     * Pass it to `CancelDeferredCharge` to stop a scheduled charge before it fires.
     * @internal
     * @format GUID
     * @readonly
     */
    deferredChargeId?: string | null;
}
/** @oneof */
interface DeferredChargeInfoStatusDetailsOneOf {
    /** Reason the charge was declined. */
    declinedOptions?: DeferredChargeDeclineReasonWithLiterals;
}
interface QueryExtendedBookingRequest {
    ...
    /**
     * Whether to return information about the waiver.
     *
     * You must have the `WAIVER_TEMPLATE.VIEWS` and `WAIVER_TEMPLATE.VIEWS` and `WAIVER.VIEW` permissions scope
     * when passing `true`.
     *
     * Default: `false`.
     * @internal
    ...
    /**
     * Whether to return event notes for each booking.
     *
     * You must have the `CALENDAR.EVENT_READ_PI` permission scope
     * when passing `true`.
     *
     ...
     * @internal
     */
    withEventNotes?: boolean;
    /**
     * Whether to return deferred charge information for the bookings.
     *
     * Default: `false`.
     * @internal
     */
    withDeferredCharge?: boolean;
    /**
...
interface SearchExtendedBookingsRequest {
    ...
    /** Whether to retrieve information about booking policy settings. */
    withBookingPolicySettings?: boolean;
    /** Whether to retrieve information about booking fee statuses. */
    withBookingFeeDetails?: boolean;
    /**
     * Whether to return information about the order from ecom.
     * @internal
...
interface SearchExtendedBookingsResponse {
    /** Retrieved bookings (empty when only aggregation is needed with limit=0). */
    extendedBookings?: ExtendedBooking[];
    /** Paging metadata. */
    pagingMetadata?: CursorPagingMetadata;
    /** Aggregation data results. */
...
interface GeotileGridBucket {
    ...
    centroid?: CommonAddressLocation;
}
...
/** Geotile grid aggregation results — one bucket per map tile at the requested precision. */
interface GeotileGridResults {
    /**
     * List of tile buckets.
     * @maxSize 1000
     */
    results?: GeotileGridBucket[];
}
interface AggregationResults extends AggregationResultsResultOneOf {
    ...
    /** Geotile grid aggregation results. */
    geotileGrid?: GeotileGridResults;
    /**
 ...
 * @deprecated
 */
declare function query(query: QueryV2, options?: QueryOptions): Promise<NonNullablePaths<QueryExtendedBookingResponse, `extendedBookings` | `extendedBookings.${number}.booking.totalParticipants` | `extendedBookings.${number}.booking.status` | `extendedBookings.${number}.booking.paymentStatus` | `extendedBookings.${number}.booking.selectedPaymentOption` | `extendedBookings.${number}.booking.createdBy.anonymousVisitorId` | `extendedBookings.${number}.booking.createdBy.memberId` | `extendedBookings.${number}.booking.createdBy.wixUserId` | `extendedBookings.${number}.booking.createdBy.appId` | `extendedBookings.${number}.booking.multiServiceBookingInfo.type` | `extendedBookings.${number}.allowedActions.cancel` | `extendedBookings.${number}.allowedActions.reschedule` | `extendedBookings.${number}.attendance.status` | `extendedBookings.${number}.attendance.numberOfAttendees` | `extendedBookings.${number}.bookingFeeDetails.cancellationFeeStatus` | `extendedBookings.${number}.order.number` | `extendedBookings.${number}.order.status` | `extendedBookings.${number}.order.paymentStatus` | `extendedBookings.${number}.order.totalPrice.amount` | `extendedBookings.${number}.order.totalPrice.formattedAmount` | `extendedBookings.${number}.order.bookingPriceDescription.original` | `extendedBookings.${number}.order.balance.amount` | `extendedBookings.${number}.order.balance.formattedAmount` | `extendedBookings.${number}.waiver.status` | `extendedBookings.${number}.subscriptionInfo.currentCycle` | `extendedBookings.${number}.subscriptionInfo.totalCycles` | `extendedBookings.${number}.subscriptionInfo.billingFrequency` | `extendedBookings.${number}.subscriptionInfo.status` | `extendedBookings.${number}.subscriptionInfo.cyclePrice.amount` | `extendedBookings.${number}.subscriptionInfo.cyclePrice.currency`, 4>>;
 ...
 */
declare function queryExtendedBookings(options?: QueryExtendedBookingsOptions): ExtendedBookingsQueryBuilder;
interface QueryExtendedBookingsOptions {
    /** Whether to return information about the actions the customer can perform for the bookings. */
    withBookingAllowedActions?: boolean | undefined;
    /** Whether to return information about the attendance for the bookings. */
    withBookingAttendanceInfo?: boolean | undefined;
    /** Whether to return information about the online conferencing details for the bookings. */
    withBookingConferencingDetails?: boolean | undefined;
    /**
     * Whether to retrieve information about booking policy settings.
     *
     * You must have the `BOOKINGS.BOOKING_POLICY_SNAPSHOT_READ` permission scope
     ...
     * Default: `false`.
     */
    withBookingPolicySettings?: boolean | undefined;
    /**
     * Whether to retrieve information about [booking fee](https://dev.wix.com/docs/api-reference/business-solutions/bookings/pricing/booking-fees/introduction)
     * statuses.
     *
     ...
     * Default: `false`
     */
    withBookingFeeDetails?: boolean | undefined;
    /**
     * Whether to return information about the order from ecom.
     *
     * Learn more about [eCommerce orders](https://dev.wix.com/docs/api-reference/business-solutions/e-commerce/orders/introduction).
     ...
     * Default: `false`.
     */
    withEcomOrder?: boolean | undefined;
    /**
     * Whether to return information about the waiver.
     *
     * You must have the `WAIVER_TEMPLATE.VIEWS` and `WAIVER_TEMPLATE.VIEWS` and `WAIVER.VIEW` permissions scope
     ...
     * @internal
     */
    withEcomTransactions?: boolean | undefined;
    /**
     * Whether to return information about the form submission.
     *
     * You must have the `BOOKINGS.BOOKING_FORM_SCHEMA_READ` permission scope
     ...
     * @internal
     */
    withFormSubmissions?: boolean | undefined;
    /**
     * Whether to return event notes for each booking.
     *
     * You must have the `CALENDAR.EVENT_READ_PI` permission scope
     ...
     * @internal
     */
    withEventNotes?: boolean | undefined;
    /**
     * Whether to return deferred charge information for the bookings.
     *
     * Default: `false`.
     * @internal
     */
    withDeferredCharge?: boolean | undefined;
    /**
     * IANA timezone for server-side adjusted datetime conversion. When set, `adjustedStart`/`adjustedEnd` are converted to this timezone. Otherwise, the slot or schedule timezone is used.
     * @internal
     * @maxLength 64
...
interface ExtendedBookingsQueryResult extends QueryCursorResult {
    items: ExtendedBooking[];
    query: ExtendedBookingsQueryBuilder;
    next: () => Promise<ExtendedBookingsQueryResult>;
    prev: () => Promise<ExtendedBookingsQueryResult>;
}
interface ExtendedBookingsQueryBuilder {
    /** @param limit - Number of items to return, which is also the `pageSize` of the results object. */
    limit: (limit: number) => ExtendedBookingsQueryBuilder;
    /** @param cursor - A pointer to specific record */
    skipTo: (cursor: string) => ExtendedBookingsQueryBuilder;
    find: () => Promise<ExtendedBookingsQueryResult>;
}
/**
 * @hidden
 * @fqn com.wixpress.bookings.reader.v2.BookingsReader.QueryExtendedBookings
 * @requiredField query
 */
declare function typedQueryExtendedBookings(query: ExtendedBookingQuery, options?: QueryExtendedBookingsOptions): Promise<NonNullablePaths<QueryExtendedBookingsResponse, `extendedBookings` | `extendedBookings.${number}.booking.totalParticipants` | `extendedBookings.${number}.booking.status` | `extendedBookings.${number}.booking.paymentStatus` | `extendedBookings.${number}.booking.selectedPaymentOption` | `extendedBookings.${number}.booking.createdBy.anonymousVisitorId` | `extendedBookings.${number}.booking.createdBy.memberId` | `extendedBookings.${number}.booking.createdBy.wixUserId` | `extendedBookings.${number}.booking.createdBy.appId` | `extendedBookings.${number}.booking.multiServiceBookingInfo.type` | `extendedBookings.${number}.allowedActions.cancel` | `extendedBookings.${number}.allowedActions.reschedule` | `extendedBookings.${number}.attendance.status` | `extendedBookings.${number}.attendance.numberOfAttendees` | `extendedBookings.${number}.bookingFeeDetails.cancellationFeeStatus` | `extendedBookings.${number}.order.number` | `extendedBookings.${number}.order.status` | `extendedBookings.${number}.order.paymentStatus` | `extendedBookings.${number}.order.totalPrice.amount` | `extendedBookings.${number}.order.totalPrice.formattedAmount` | `extendedBookings.${number}.order.bookingPriceDescription.original` | `extendedBookings.${number}.order.balance.amount` | `extendedBookings.${number}.order.balance.formattedAmount` | `extendedBookings.${number}.waiver.status` | `extendedBookings.${number}.subscriptionInfo.currentCycle` | `extendedBookings.${number}.subscriptionInfo.totalCycles` | `extendedBookings.${number}.subscriptionInfo.billingFrequency` | `extendedBookings.${number}.subscriptionInfo.status` | `extendedBookings.${number}.subscriptionInfo.cyclePrice.amount` | `extendedBookings.${number}.subscriptionInfo.cyclePrice.currency`, 4>>;
interface ExtendedBookingQuerySpec extends QuerySpec {
    paging: 'cursor';
    wql: [];
}
type CommonQueryWithEntityContext = Query<ExtendedBooking, ExtendedBookingQuerySpec>;
type ExtendedBookingQuery = {
    /**
    Cursor token pointing to a page of results. In the first request,
    specify `cursorPaging.limit`. For following requests, specify the
    retrieved `cursorPaging.cursor` token and not `query.filter` or
...
declare const utils: {
    query: _wix_sdk_types.QueryHelpers<ExtendedBooking, ExtendedBookingQuerySpec, ExtendedBookingQuery>;
};
/**
 ...
 * @public
 * @permissionId bookings:v2:booking:count_extended_bookings
 * @permissionId BOOKINGS.MANAGE_BOOKINGS
 * @permissionId BOOKINGS.BOOKING_READ_ANY
 * @applicableIdentity APP
 * @fqn com.wixpress.bookings.reader.v2.BookingsReader.CountExtendedBookings
 */
declare function countExtendedBookings(options?: CountExtendedBookingsOptions): Promise<NonNullablePaths<CountExtendedBookingsResponse, `count`, 0>>;
interface CountExtendedBookingsOptions {
    /**
     * Filter object in the following format:
     * `"filter" : {
     * "fieldName1": "value1",
...
export { Actor, type ActorWithLiterals, type AdditionalFee, type Address, type AddressLocation, type AddressStreetOneOf, type Aggregation, type AggregationData, type AggregationKindOneOf, type AggregationResults, type AggregationResultsResultOneOf, type AggregationResultsScalarDateResult, type AggregationResultsScalarResult, AggregationType, type AggregationTypeWithLiterals, type AllowedActions, type Attendance, AttendanceStatus, type AttendanceStatusWithLiterals, type Balance, type BookedAddOn, type BookedEntity, type BookedEntityItemOneOf, type BookedResource, type BookedSchedule, type BookedSlot, type Booking, type BookingFeeDetails, BookingFeeStatus, type BookingFeeStatusWithLiterals, type BookingParticipantsInfoOneOf, type BookingPolicySettings, type BookingSource, BookingStatus, type BookingStatusWithLiterals, type CommonAddressLocation, CommonAggregationType, type CommonAggregationTypeWithLiterals, type CommonPaging, type CommonQueryV2, type CommonQueryV2PagingMethodOneOf, type CommonQueryWithEntityContext, CommonScalarType, type CommonScalarTypeWithLiterals, type ConferencingDetails, type ContactDetails, type CountExtendedBookingsOptions, type CountExtendedBookingsRequest, type CountExtendedBookingsResponse, type CouponInfo, type CursorPaging, type CursorPagingMetadata, type CursorSearch, type CursorSearchPagingMethodOneOf, type Cursors, type CustomFormField, type CyclePrice, type DateHistogramAggregation, type DateHistogramResult, type DateHistogramResults, type DateTimeRule, DeferredChargeDeclineReason, type DeferredChargeDeclineReasonWithLiterals, type DeferredChargeInfo, type DeferredChargeInfoStatusDetailsOneOf, DeferredChargeStatus, type DeferredChargeStatusWithLiterals, type Duration, DurationUnitType, type DurationUnitTypeWithLiterals, type ExtendedBooking, type ExtendedBookingQuery, type ExtendedBookingQuerySpec, type ExtendedBookingsQueryBuilder, type ExtendedBookingsQueryResult, type ExtendedFields, type FlowControlSettings, type FormSubmission, Frequency, type FrequencyWithLiterals, type GeotileGridBucket, type GeotileGridResults, type GroupByAggregation, type GroupByAggregationKindOneOf, type GroupByValueResults, type IdentificationData, type IdentificationDataIdOneOf, IdentityType, type IdentityTypeWithLiterals, type IncludeMissingValuesOptions, Interval, type IntervalWithLiterals, type Location, LocationType, type LocationTypeWithLiterals, type MembershipName, type MembershipPaymentCredits, type MembershipPaymentDetails, MissingValues, type MissingValuesWithLiterals, Mode, type ModeWithLiterals, type MultiServiceBookingInfo, MultiServiceBookingType, type MultiServiceBookingTypeWithLiterals, type NestedAggregation, type NestedAggregationItem, type NestedAggregationItemKindOneOf, type NestedAggregationResults, type NestedAggregationResultsResultOneOf, NestedAggregationType, type NestedAggregationTypeWithLiterals, type NestedResultValue, type NestedResultValueResultOneOf, type NestedResults, type NestedValueAggregationResult, type Order, OrderStatus, type OrderStatusWithLiterals, type OrderTransactions, type Paging, type PagingMetadataV2, type ParticipantChoices, type ParticipantNotification, type ParticipantsRange, PaymentStatus, PaymentStatusEnumPaymentStatus, type PaymentStatusEnumPaymentStatusWithLiterals, type PaymentStatusWithLiterals, type PhoneCall, Platform, type PlatformWithLiterals, type Price, type PriceDescription, type QueryExtendedBookingRequest, type QueryExtendedBookingResponse, type QueryExtendedBookingsOptions, type QueryExtendedBookingsRequest, type QueryExtendedBookingsResponse, type QueryOptions, type QueryV2, type QueryV2PagingMethodOneOf, type RangeAggregation, type RangeAggregationResult, type RangeBucket, type RangeResult, type RangeResults, RequestedGender, type RequestedGenderValue, type RequestedGenderWithLiterals, type ResourceSelection, ResourceTransparency, type ResourceTransparencyWithLiterals, type Results, type ScalarAggregation, type ScalarDateResult, type ScalarResult, ScalarType, type ScalarTypeWithLiterals, type SearchDetails, type SearchExtendedBookingsRequest, type SearchExtendedBookingsResponse, SelectedPaymentOption, type SelectedPaymentOptionWithLiterals, SelectionMethod, type SelectionMethodWithLiterals, type ServiceChoice, type ServiceChoiceChoiceOneOf, type ServiceChoices, SortDirection, type SortDirectionWithLiterals, SortOrder, type SortOrderWithLiterals, SortType, type SortTypeWithLiterals, type Sorting, Status, type StatusWithLiterals, type StreetAddress, type Subdivision, type SubscriptionInfo, SubscriptionStatus, type SubscriptionStatusWithLiterals, type ValueAggregation, type ValueAggregationOptionsOneOf, type ValueAggregationResult, type ValueResult, type ValueResults, ValueType, type ValueTypeWithLiterals, type Waiver, type ZonedDate, countExtendedBookings, query, queryExtendedBookings, typedQueryExtendedBookings, utils };
