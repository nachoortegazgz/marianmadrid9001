...
interface ConsentPolicy {
     ...
     * > To stay compatible with future changes, migrate to [`getCurrentConsentPolicy()`](https://dev.wix.com/docs/sdk/frontend-modules/site/consent-policy-manager/get-current-consent-policy).
     * >
     ...
     * >
     * >    ```javascript
     * >    import { consentPolicy } from "@wix/site";
     * >    ```
     * >
     * > 2. If you plan to migrate all methods that use `getCurrentConsentPolicy()`,
     * >    remove the original `import { consentPolicy } from "wix-window-frontend";` statement.
     * >
     * > 3. Look for any code that uses `consentPolicy.getCurrentConsentPolicy()`,
     ...
     * > #### Migration Instructions
     * > If this method is already in your code, it will continue to work.
     * > To stay compatible with future changes, migrate to [`onConsentPolicyChanged()`](https://dev.wix.com/docs/sdk/frontend-modules/site/consent-policy-manager/on-consent-policy-changed).
     * >
     * > To migrate to the new method:
     * >
     * > 1. Add the new import statement:
     ...
     * >    ```
     * >
     * > 2. If you plan to migrate all methods that use `onConsentPolicyChanged()`,
     * >    remove the original `import { consentPolicy } from "wix-window-frontend";` statement.
     * >
     * > 3. Look for any code that uses `consentPolicy.onConsentPolicyChanged()`,
     * >    and replace it with the new import.
     ...
     * @param handler - The name of the method to run when the consent policy changes.
     ...
     * > #### Migration Instructions
     * > If this method is already in your code, it will continue to work.
     * > To stay compatible with future changes, migrate to [`resetConsentPolicy()`](https://dev.wix.com/docs/sdk/frontend-modules/site/consent-policy-manager/reset-consent-policy).
     * >
     * > To migrate to the new method:
     * >
     * > 1. Add the new import statement:
     ...
     * >    ```
     * >
     * > 2. If you plan to migrate all methods that use `resetConsentPolicy()`,
     * >    remove the original `import { consentPolicy } from "wix-window-frontend";` statement.
     * >
     * > 3. Look for any code that uses `consentPolicy.resetConsentPolicy()`,
     * >    and replace it with the new import.
     ...
     * > #### Migration Instructions
     * > If this method is already in your code, it will continue to work.
     * > To stay compatible with future changes, migrate to [`setConsentPolicy()`](https://dev.wix.com/docs/sdk/frontend-modules/site/consent-policy-manager/set-consent-policy).
     * >
     * > To migrate to the new method:
     * >
     * > 1. Add the new import statement:
     ...
     * >    ```
     * >
     * > 2. If you plan to migrate all methods that use `setConsentPolicy()`,
     * >    remove the original `import { consentPolicy } from "wix-window-frontend";` statement.
     * >
     * > 3. Look for any code that uses `consentPolicy.setConsentPolicy()`,
     * >    and replace it with the new import.
     ...
     * @returns Fulfilled - All the details of a site visitor's consent policy, including if the current policy is the default site policy and the policy's allowed cookies.
     * Rejected - Error message.
     * @servicePath wix-window-frontend.ConsentPolicy.PolicyDetails
     */
 ...
 *
 *  To learn more about popups, see the [Hello Popups](https://dev.wix.com/docs/coding-examples/getting-started/hello-world/hello-popups) example page.
 *
...
interface Lightbox {
     ...
     * > **Note:** The terms popup and lightbox refer to the same element. While the editor and dashboard now refer to it as a [popup](https://support.wix.com/en/article/studio-editor-using-popups), the API methods continue to use the term lightbox for backward compatibility. The documentation uses both terms accordingly.
     *
     *  The `close()` method closes a popup and allows you to pass data from the popup to the page that opened the popup.
     *  If a popup is closed through the 'X' icon, close button, or popup overlay, data will not be returned to the original page.
     *
     ...
     *  2. Disable these closing options in the popup's settings under [**Set Triggers**](https://support.wix.com/en/article/studio-editor-using-lightboxes#setting-triggers-for-a-lightbox).
     ...
     *
     *  To pass data to a popup, it must be opened programmatically using the [`openLightbox()`](https://dev.wix.com/docs/velo/api-reference/wix-window-frontend/open-lightbox)
     *  method. Popups that are opened automatically on page load, or with a link from a page element don't receive passed data.
     *
     * @returns The data object that was passed to the popup.
     */
     ...
     * > **Note:** The terms popup and lightbox refer to the same element. While the editor and dashboard now refer to it as a [popup](https://support.wix.com/en/article/studio-editor-using-popups), the API methods continue to use the term lightbox for backward compatibility. The documentation uses both terms accordingly.
     *
     *  The `open()` method opens an app popup by its extension ID and optionally passes data to it. Use this in your app's [site widget](https://dev.wix.com/docs/build-apps/develop-your-app/extensions/site-extensions/site-widgets/about-site-widget-extensions) to trigger the app's popup. Learn more about [site popup extensions](https://dev.wix.com/docs/build-apps/develop-your-app/extensions/site-extensions/site-lightboxes/about-site-lightbox-extensions).
     *
     *  To access the data passed to a popup, call [`getContext()`](https://dev.wix.com/docs/sdk/frontend-modules/window/lightbox/get-context) in the popup's code.
     *
     *  If the popup is closed programmatically using [`close()`](https://dev.wix.com/docs/sdk/frontend-modules/window/lightbox/close)
     *  and includes data, the promise returned by `open()` resolves with that data.
     *
     *  Learn more about [opening and closing a popup from your app's code](https://dev.wix.com/docs/build-apps/develop-your-app/extensions/site-extensions/site-lightboxes/about-site-lightbox-extensions#open-and-close-a-lightbox-from-your-apps-code)
     * @param options - Open popup options.
     * @servicePath wix-window-frontend.LightboxOpenOptions
     * @returns Fulfilled - Resolves with data returned from the popup when it's closed.
     ...
     * @param backgroundScrollEnabled - Whether site visitors can scroll the site in the background behind a popup while a popup overlay is open.
     *
     * Default: `false`
     */
    setBackgroundScroll(backgroundScrollEnabled?: boolean): Promise<void>;
}
/**
 * Closes a popup.
 *
 ...
 * Default: `false`
 */
declare function setBackgroundScroll(backgroundScrollEnabled?: boolean): Promise<void>;
...
declare const lightboxSdkModuleContext_setBackgroundScroll: typeof setBackgroundScroll;
declare namespace lightboxSdkModuleContext {
  export { type lightboxSdkModuleContext_Lightbox as Lightbox, lightboxSdkModuleContext_close as close, lightboxSdkModuleContext_getContext as getContext, lightboxSdkModuleContext_open as open, lightboxSdkModuleContext_setBackgroundScroll as setBackgroundScroll };
}
 ...
 *
 *  Set the current language using a 2-letter language code. The code must
 *  represent one of [the languages set to show on a site](https://support.wix.com/en/article/wix-multilingual-managing-languages-in-a-multilingual-site).
 *  You can retrieve a site's languages and corresponding language codes using
 *  the `siteLanguages` property.
 *
 *  Getting the `currentLanguage` property gets the 2-letter language code
 ...
 *
 * A multilingual site is a site that's set up to display in more than 1
 *  language. To learn more about enabling multilingual functionality in a
 *  site, see [Wix Multilingual: An Overview](https://support.wix.com/en/article/wix-multilingual-an-overview).
 *
 ...
 *  > + Backend rendering only occurs when site visitors initially enter a site, and not when sitevisitors navigate from page-to-page within a site.
 *
...
interface WarmupData {
     ...
     *  > **Notes:**
     *  > + Rendering never occurs server-side when previewing a site.
     *  > + Server-side rendering only occurs when site visitors initially enter a site, and not when site visitors navigate from page-to-page in a site.
     ...
     *  > + Rendering never occurs server-side when previewing a site.
     *  > + Server-side rendering only occurs when site visitors initially enter a site, and not when site visitors navigate from page-to-page in a site.
     * @param key - Name representing the data to return to the client-side. Key names are unique in a site. Using the same key name in the same site code overwrites the corresponding data.
     * @requiredField key
 ...
 * __Important:__
 *
 * Some tablet devices, such as iPads, are identified in this property as "Desktop".
 *
 * @readonly
 */
 ...
 *
 *  > **Note:** When site visitors move from page to page in a site, the `referrer` property
 ...
 *
 * `copyToClipboard()` copies the specified text to a site visitor's clipboard.
 *
 ...
 * For more information, see App Page Data ([SDK](https://dev.wix.com/docs/sdk/frontend-modules/window/app-page-data) | [Velo](https://dev.wix.com/docs/velo/api-reference/wix-window-frontend/app-page-data)).
 *
 * Learn more about [building custom app pages](https://dev.wix.com/docs/develop-websites/articles/wix-apps/replace-a-wix-business-app-page-with-your-own-custom-version).
 *
 * >**Note:** If you call `getAppPageData()` for a page that's not
 ...
 *
 *  + On Chrome, the method only works on HTTPS sites.
 *  + On Chrome, Firefox, and Safari, the method only works if the site visitor
 *      approves a popup. If they don't approve, the promise is rejected.
 ...
 *  2. Set **Automatically display popup on pages** to **No** in the popup's settings under [**Set Triggers**](https://support.wix.com/en/article/studio-editor-using-lightboxes#setting-triggers-for-a-lightbox).
 *
 *  If you pass data to a popup, call `getContext()`
 ...
 * @returns Fulfilled - The returned data from the popup.
 * Rejected - The error that caused the rejection.
 */
 ...
 *  a current page. Unlike a popup, which
 *  is opened by calling [`openLightbox()`](https://dev.wix.com/docs/sdk/frontend-modules/window/open-lightbox) method, a window
 *  opened by `openModal()` isn't part of a site's structure.
 *
 *  Only 1 modal window can be open at any given time. Therefore, opening
 ...
 *  event to read the `data` property of the received `MessageEvent`
 *  to receive the message on the parent page.
 ...
 * > #### Migration Instructions
 * > If this method is already in your code, it will continue to work.
 * > To stay compatible with future changes, migrate to [`registerEventListener()`](https://dev.wix.com/docs/sdk/host-modules/site/analytics/register-event-listener).
 * >
 * > To migrate to the new method:
 * >
 * > 1. Add the new import statement:
 ...
 * >    ```
 * >
 * > 2. If you plan to migrate all methods that use `registerEventListener()`,
 * >    remove the original `import { window } from "@wix/site-window";` statement.
 * >
 * > 3. Look for any code that uses `window.registerEventListener()`,
 * >    and replace it with `analytics.registerEventListener()`.
 * >    Update your code to work with the new `registerEventListener()`
 * >    call and response properties.
 * >
 * > 4. Test your changes to make sure your code behaves as expected.
 * >
 ...
 * > [`registerEventListener()`](https://dev.wix.com/docs/sdk/host-modules/site/analytics/register-event-listener).
 * >
 * > To migrate to the new method:
 * >
 * > 1. Add the new import statement:
 ...
 *
 * The `x` and `y` parameters determine the top-left pixel that's
 ...
 * > #### Migration Instructions
 * > If this method is already in your code, it will continue to work.
 * > To stay compatible with future changes, migrate to [`trackEvent()`](https://dev.wix.com/docs/sdk/host-modules/site/analytics/track-event).
 * >
 ...
 * >    ```
 * >
 * > 2. If you plan to migrate all functions that use `trackEvent()`,
 * >    remove the original `import { window } from "@wix/site-window";` statement.
 * >
 * > 3. Look for any code that uses `window.trackEvent()`,
 * >    and replace it with `analytics.trackEvent()`.
 * >    Update your code to work with the new `trackEvent()`
 * >    call and response properties.
 * >
 ...
 *
 * > **Note:** This method only runs on published versions of a site.
 * It doesn't work when previewing a site.
 *
 ...
 * + `ViewContent`
 * @requiredField eventName
 * @param parameters - The event's parameters. **Note:** The `CompleteRegistration` and `Schedule` events don't take any parameters.
