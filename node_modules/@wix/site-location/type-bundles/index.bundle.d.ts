...
interface QueryParams {
     ...
     *
     *  The `add()` method can only be used when browser
     ...
     *  newly specified value overwrites the key's previous value.
     *
     *  Calling the `add()` method triggers `onChange()`
     ...
     *
     *  The `remove()` method can only be used when browser
     *  [rendering](https://dev.wix.com/docs/develop-websites/articles/coding-with-velo/frontend-code/page-rendering/about-page-rendering) happens,
     *  meaning you can only use it in frontend code after the page is ready.
     *
     *  If a specified key doesn't exist as a query parameter, it
     *  is ignored.
     *
     *  Calling the `remove()` method triggers `onChange()`
     *  if it has been registered.
     *  > **Note:** To retrieve the page's current query parameters, use the
     *  `query` method.
 ...
 * If the page is a dynamic page or a router page, the `prefix` appears after the base URL, before the path.
 *
 *
 *  Premium sites:
 ...
 *
 *  To learn more about dynamic page prefixes, see [About URL Prefixes and Page Grouping of Dynamic Pages](https://dev.wix.com/docs/develop-websites/articles/databases/wix-data/dynamic-pages/making-dynamic-page-urls-meaningful-with-prefixes).
 *
 *  To learn more about router page prefixes, see [About Routers](https://dev.wix.com/docs/develop-websites/articles/coding-with-velo/routers/about-routers#url-prefix).
 * @readonly
 */
 ...
 *
 * The event handler set by the `onChange()` method runs when the location changes
 * but the change doesn't trigger navigation. This situation occurs when navigating between
 * subitems on a page that's managed by a full-page application.
 *
 * For example, a store product page is a full-page application. When a product page's path
 * changes because it's switching between items, no actual navigation is taking place. You
 ...
 *
 * The `onChange()` method can only be used when browser
 * [rendering](https://dev.wix.com/docs/develop-websites/articles/coding-with-velo/frontend-code/page-rendering/about-page-rendering) happens,
 * meaning you can only use it in frontend code after the page is ready.
 *
 * To determine if a page is managed by a full-page application, use the Site API's
 * `currentPage` property or `getSiteStructure()`
 * method to retrieve a `StructurePage` object that
 * corresponds to the page. If the object contains an `applicationId` value, then the
 * page is managed by a full-page application.
 * @param handler - The method to call when the location changes.
 ...
 *
 * The `to()` method navigates the browser to another web page.
 *
 *  The `to()` method can only be used when browser
 *  [rendering](https://dev.wix.com/docs/develop-websites/articles/coding-with-velo/frontend-code/page-rendering/about-page-rendering) happens,
 *  meaning you can only use it in frontend code after the page is ready.
 *
 *  The following link patterns are supported:
 *
 *  + `/localPageURL`: Another page on a site.
 *  + `/localPageURL#`: Another page on a site scrolled to the
 *     element with the specified ID. The element must be an element that supports
 *     the `scrollTo` method.
 *  + `/localPageURL?queryParam=value`: Another page on a site with query parameters.
 *  + `/`: A site's home page.
 ...
 *
 *
 *  To find the local URL of a page on a site in the editor:
 *
 ...
 *
 *    For example, if the URL structure of a dynamic page looks like:
 *
 ...
 *    is /Recipes/Waffles.
 *  + Router page: You can't navigate directly to a specific router page. You
 ...
 *
 *  The `to()` method attempts to properly encode the URL parameter that
 *  is passed to it. For example, `.../some page` is encoded to
 *  `.../some%20page`. However, some URLs don't have 1 unambiguous encoding.
 *  In those cases it's up to you to encode the URL to reflect your intentions.
 *  Because of these situations, it's a best practice to always encode URLs
 *  before you pass them to the `to()` method.
 *
 *  Note that Wix URLs don't contain spaces. A page which has spaces in its
 *  name has its spaces replaced with dashes (`-`). Similarly, a dynamic page
 *  whose URL contains the value of a field in a collection with spaces
 *  has its spaces replaced with dashes (`-`).
 *
 *  > **Note:** The `to()` method doesn't work while previewing a site.
