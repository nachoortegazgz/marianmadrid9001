import { HttpClient, NonNullablePaths, MaybeContext, BuildRESTFunction } from '@wix/sdk-types';
import { ListCustomFieldSuggestionsOptions, ListCustomFieldSuggestionsResponse, SuggestionsQueryBuilder, CustomFieldSuggestionQuery, typedQueryCustomFieldSuggestions } from './index.typings.mjs';
export { AddressLocation, CommonQueryWithEntityContext, CustomFieldSuggestion, CustomFieldSuggestionQuerySpec, ListCustomFieldSuggestionsRequest, Origin, OriginWithLiterals, Paging, PagingMetadata, Privacy, PrivacyWithLiterals, Query, QueryCustomFieldSuggestionsRequest, QueryCustomFieldSuggestionsResponse, SocialTypeType, SocialTypeTypeWithLiterals, SortOrder, SortOrderWithLiterals, Sorting, SuggestionsQueryResult, Type, TypeWithLiterals, utils } from './index.typings.mjs';

declare function listCustomFieldSuggestions$1(httpClient: HttpClient): ListCustomFieldSuggestionsSignature;
interface ListCustomFieldSuggestionsSignature {
    /**
     * Retrieves a list of suggestions for both custom fields and existing fields available from the Wix Members Area.
     */
    (options?: ListCustomFieldSuggestionsOptions): Promise<NonNullablePaths<ListCustomFieldSuggestionsResponse, `suggestions` | `suggestions.${number}.defaultPrivacy` | `suggestions.${number}.fieldType` | `suggestions.${number}.socialType` | `suggestions.${number}.fieldOrigin`, 2>>;
}

declare function customQueryCustomFieldSuggestions(httpClient: HttpClient): {
    (): SuggestionsQueryBuilder;
    (query: CustomFieldSuggestionQuery): ReturnType<typeof typedQueryCustomFieldSuggestions>;
};
declare const listCustomFieldSuggestions: MaybeContext<BuildRESTFunction<typeof listCustomFieldSuggestions$1> & typeof listCustomFieldSuggestions$1>;
declare const queryCustomFieldSuggestions: MaybeContext<BuildRESTFunction<typeof customQueryCustomFieldSuggestions> & typeof customQueryCustomFieldSuggestions>;

export { CustomFieldSuggestionQuery, ListCustomFieldSuggestionsOptions, ListCustomFieldSuggestionsResponse, SuggestionsQueryBuilder, listCustomFieldSuggestions, queryCustomFieldSuggestions };
