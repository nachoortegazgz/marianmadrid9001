...
import { AuthenticationStrategy, type IdentityProvider, type Token, type AccessToken, type RefreshToken, type Tokens, type TokenStorage } from '@wix/sdk-types';
import { LoginState } from '@wix/sdk-runtime/login-state';
export { LoginState };
export type { Token, AccessToken, RefreshToken, Tokens, TokenStorage };
/**
...
export interface IOAuthStrategy extends AuthenticationStrategy {
    ...
    /**
     * Retrieves and authenticates site member's access and refresh tokens given the member's
     * email, creating or updating the member as needed (Sign On).
     *
     * Unlike `getMemberTokensForExternalLogin`, the returned tokens are bound to a new identity
     * session, so they can be used for session transfer to Wix-managed pages (e.g. redirecting
     * to a Wix-managed checkout using the Redirects API).
     *
     * In addition to the email, an API Key that is permitted to call the Sign On API and to
     * create member tokens is required (this is an administrative API that bypasses the need
     * for member credentials and so can only be used from confidential clients).
     * @param email The member's login email to get the tokens for
     * @param apiKey An API Key that is permitted to call the Sign On API and to create member tokens
     * @param options Optional Sign On options (profile, verifyEmail, mergeExistingContact)
     * @returns Tokens (access and refresh) for the member, bound to a new identity session
     */
    getMemberTokensForExternalLoginWithSession: (email: string, apiKey: string, options?: authentication.SignOnOptions) => Promise<Tokens>;
    sessions: {
